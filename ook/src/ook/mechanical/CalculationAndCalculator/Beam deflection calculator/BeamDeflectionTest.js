// import React, { useState, useEffect } from 'react';
// import { XYPlot, LabelSeries, LineSeries, } from 'react-vis';
// import 'react-vis/dist/style.css';
// import { MdDelete } from "react-icons/md";
// import Backgroundimg from '../image folder/Deflaction2.jpg'
// import BeamDeflection from './Beam Deflection dropdown/Beam Deflection'
// import Load from './Beam Deflection dropdown/Load'
// import './Beam Deflection dropdown/Beam Deflection.css'
// import { Link } from 'react-router-dom'
// import LengthOfBeam from './BeamDeflactionCalculator/LengthOfBeam'
// import YoungsModules from './BeamDeflactionCalculator/YoungsModules'
// import CrossSection from './BeamDeflactionCalculator/CrossSection'
// import SupportPage from './BeamDeflactionCalculator/Support'
// import LoadPage from './BeamDeflactionCalculator/Load'
// import SolutionsOfBeamDeflaction from './SolutionsOfBeamDeflaction';
// import { ResponsiveContainer, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip } from 'recharts';

// export default function BeamDeflectionCalculator() {

//   const [selectedOption, setSelectedOption] = useState("option1");
//   const handleOptionChange = (option) => {
//     setSelectedOption(option)
//   }

//   const [isActive3, setIsActive3] = useState(true);

//   const toggleClass3 = () => {
//     setIsActive3(previsActive3 => !previsActive3);

//   };
//   const [MetricOrImperial, setMetricOrImperial] = useState("option1");

//   const toggleMetricOrImperial = (option) => {
//     setMetricOrImperial(option);

//   };
//   const [expanded, setExpanded] = useState(false);
//   const [Sectionthird, setSectionthird] = useState(false);

//   useEffect(() => {
//     const handleScroll = () => {
//       if (window.scrollY > 100 && !expanded) {
//         setExpanded(true);
//       }
//       if (window.scrollY > 350 && !Sectionthird) {
//         setSectionthird(true);
//       }
//     };

//     window.addEventListener('scroll', handleScroll);

//     return () => {
//       window.removeEventListener('scroll', handleScroll);
//     };
//   }, [expanded, Sectionthird]);





//   // calculator functions
//   const MetricUnits = ['mm', 'm'];
//   const MetricConversionFactors = {
//     mm: [1, 0.001],
//     m: [1000, 1],
//   };
//   const [metriclength, setmetriclength] = useState('');
//   const [metriclengthSelectedUnit, setmetriclengthSelectedUnit] = useState('mm');
//   const [internalMetricValue, setInternalMetricValue] = useState(0); // Always in mm
//   const handleMetricInputChange = (value) => {
//     setmetriclength(value);
//     const factor = MetricConversionFactors[metriclengthSelectedUnit][0];
//     setInternalMetricValue(parseFloat(value) * factor);
//   };

//   const handleMetricUnitChange = (unit) => {
//     const newMetricValueInMM = parseFloat(metriclength) * MetricConversionFactors[metriclengthSelectedUnit][0];
//     const convertedValue = newMetricValueInMM / MetricConversionFactors[unit][0];
//     setmetriclengthSelectedUnit(unit);
//     setmetriclength(convertedValue.toFixed(5));



//     const lengthfactor = MetricConversionFactors[metriclengthSelectedUnit][MetricUnits.indexOf(unit)];
//     setLength((parseFloat(metriclength) * lengthfactor).toFixed(2));
//     setLengthOfBeamSelectedUnit(unit);

//     switch (unit) {
//       case 'mm':
//         setUnitText('N');
//         setUnitText2('mm');
//         break;
//       case 'm':
//         setUnitText('KN');
//         setUnitText2('m');
//         break;
//       default:
//         setUnitText('N');
//         setUnitText2('mm');
//         break;
//     }
//   };


//   const ImperialLengthunits = ['in'];
//   const ImperialFactors = {
//     in: [1],
//   };

//   const [Imperiallength, setImperiallength] = useState('');
//   const [ImperiallengthSelectedUnit, setImperiallengthSelectedUnit] = useState('in');

//   const handleImperialInputChange = (value) => {
//     setImperiallength(value);
//   };

//   const handleImperialUnitChange = (unit) => {
//     setImperiallengthSelectedUnit(unit);
//     const Imperialfactor = ImperialFactors[unit][ImperialLengthunits.indexOf(ImperiallengthSelectedUnit)];
//     setImperiallength((parseFloat(Imperiallength) / Imperialfactor).toFixed(3));


//     const lengthfactor = ImperialFactors[ImperiallengthSelectedUnit][ImperialLengthunits.indexOf(unit)];
//     setLength((parseFloat(Imperiallength) * lengthfactor).toFixed(2));
//     setLengthOfBeamSelectedUnit(unit);

//     switch (unit) {
//       case 'in':
//         setUnitText('lb');
//         setUnitText2('in');
//         break;
//       default:
//         setUnitText('lb');
//         setUnitText2('in');
//         break;
//     }
//   };

//   const [unitText, setUnitText] = useState('N');
//   const [unitText2, setUnitText2] = useState('mm');

//   const units = ['Mpa', 'Pa'];
//   const conversionFactors = {
//     Mpa: [1, 1e6],
//     Pa: [1e-6, 1],
//   };
//   const [E, setE] = useState('');
//   const [selectedUnit, setSelectedUnit] = useState('Mpa');
//   const [EInMpa, setEInMpa] = useState('');

//   const handleInputChange = (value) => {
//     setE(value);
//     const factor = conversionFactors[selectedUnit][0];
//     setEInMpa(parseFloat(value) * factor);
//   };

//   const handleUnitChange = (unit) => {
//     let newValue = parseFloat(E);
//     if (unit === 'Pa' && selectedUnit === 'Mpa') {
//       newValue *= 1e6;
//       if (newValue >= 1000) {
//         newValue = newValue.toExponential(3);
//       }
//     } else if (unit === 'Mpa' && selectedUnit === 'Pa') {
//       newValue /= 1e6;
//     }
//     setSelectedUnit(unit);
//     setE(newValue);
//   };

//   const Imperialunits = ['Psi'];
//   const [ImperialE, setImperialE] = useState('');
//   const [ImperialselectedUnit, setImperialSelectedUnit] = useState('Psi');

//   const handleImperialEInputChange = (value) => {
//     setImperialE(value);
//   };

//   const handleImperialEUnitChange = (unit) => {
//     setImperialSelectedUnit(unit);
//   };

//   if (MetricOrImperial === "option1") {
//     var Length = internalMetricValue;
//     var LengthOfBeamconversionFactors = MetricConversionFactors;
//     var LengthOfBeamSelectedUnit = metriclengthSelectedUnit;
//     var LengthOfBeamunits = MetricUnits;
//     var setLength = setmetriclengthSelectedUnit;
//     var setLengthOfBeamSelectedUnit = setmetriclengthSelectedUnit;
//     var VarE = EInMpa
//   } else if (MetricOrImperial === "option2") {
//     var Length = Imperiallength;
//     var LengthOfBeamconversionFactors = ImperialFactors;
//     var LengthOfBeamSelectedUnit = ImperiallengthSelectedUnit;
//     var LengthOfBeamunits = ImperialLengthunits;
//     var setLength = setImperiallength;
//     var setLengthOfBeamSelectedUnit = setImperiallengthSelectedUnit;
//     var VarE = ImperialE

//   }
//   const MetricAUnits = ['mm', 'm'];
//   const MetricAConversionFactors = {
//     mm: [1, 0.001],
//     m: [1000, 1],
//   };
//   const [metricAInputValue, setMetricAInputValue] = useState('');
//   const [metricASelectedUnit, setMetricASelectedUnit] = useState('mm');
//   const [internalMetricAValue, setInternalMetricAValue] = useState(0); // Always in mm

//   const handleMetricAInputChange = (value) => {
//     setMetricAInputValue(value);
//     const factor = MetricAConversionFactors[metricASelectedUnit][0];
//     setInternalMetricAValue(parseFloat(value) * factor);
//   };

//   const handleMetricAUnitChange = (unit) => {
//     const newMetricValueInMM = parseFloat(metricAInputValue) * MetricAConversionFactors[metricASelectedUnit][0];
//     const convertedValue = newMetricValueInMM / MetricAConversionFactors[unit][0];
//     setMetricASelectedUnit(unit);
//     setMetricAInputValue(convertedValue.toFixed(5));
//   };
//   const ImperialAunits = ['in'];
//   const ImperialAFactors = {
//     in: [1],
//   };

//   const [ImperialAinputValue, setImperialAInputValue] = useState('');
//   const [ImperialAselectedUnit, setImperialASelectedUnit] = useState('in');

//   const handleImperialAInputChange = (value) => {
//     setImperialAInputValue(value);
//   };

//   const handleImperialAUnitChange = (unit) => {
//     setImperialASelectedUnit(unit);
//     const Imperialfactor = ImperialAFactors[unit][ImperialAunits.indexOf(ImperialAselectedUnit)];
//     setImperialAInputValue((parseFloat(ImperialAinputValue) / Imperialfactor).toFixed(3));
//   };

//   const MetricIUnits = ['mm', 'm'];
//   const MetricIConversionFactors = {
//     mm: [1, 0.001],
//     m: [1000, 1],
//   };
//   const [metricIInputValue, setMetricIInputValue] = useState('');
//   const [metricISelectedUnit, setMetricISelectedUnit] = useState('mm');
//   const [internalMetricIValue, setInternalMetricIValue] = useState(0); // Always in mm

//   const handleMetricIInputChange = (value) => {
//     setMetricIInputValue(value);
//     const factor = MetricIConversionFactors[metricISelectedUnit][0];
//     setInternalMetricIValue(parseFloat(value) * factor);
//   };

//   const handleMetricIUnitChange = (unit) => {
//     const newMetricValueInMM = parseFloat(metricIInputValue) * MetricIConversionFactors[metricISelectedUnit][0];
//     const convertedValue = newMetricValueInMM / MetricIConversionFactors[unit][0];
//     setMetricISelectedUnit(unit);
//     setMetricIInputValue(convertedValue.toFixed(5));
//   };



//   const ImperialIunits = ['in'];
//   const ImperialIFactors = {
//     in: [1],
//   };

//   const [ImperialIinputValue, setImperialIInputValue] = useState('');
//   const [ImperialIselectedUnit, setImperialISelectedUnit] = useState('in');

//   const handleImperialIInputChange = (value) => {
//     setImperialIInputValue(value);
//   };

//   const handleImperialIUnitChange = (unit) => {
//     setImperialISelectedUnit(unit);
//     const Imperialfactor = ImperialIFactors[unit][ImperialIunits.indexOf(ImperialIselectedUnit)];
//     setImperialIInputValue((parseFloat(ImperialIinputValue) / Imperialfactor).toFixed(3));
//   };


//   const [shearForceData, setShearForceData] = useState([]);
//   const [bendingMomentData, setBendingMomentData] = useState([]);
//   const [deflectionData, setDeflectionData] = useState([]);

//   // Define supports
//   const [Supports, setSupports] = useState([{ position: 5, type: 'fixed' }]);

//   // Define point loads
//   const [pointLoads, setPointLoads] = useState([{ position: 5, magnitude: 100 }])

//   // Define varying distributed loads
//   const [Varyingdistributedload, setVaryingdistributedload] = useState({ start: 4, end: 7, startMagnitude: 15, endMagnitude: 20 })

//   // Define distributed loads
//   const [distributedload, setdistributedload] = useState({ start: 1, end: 3, distributedloadmagnitude: 120 })


//   const [reactions, setReactions] = useState({ RA: 0, RB: 0 });


//   // Varying distributed load
//   // distributed load

//   useEffect(() => {
//     calculateReactions();
//   });

//   const addPointLoad = () => {
//     setPointLoads([...pointLoads, { position: 0, magnitude: 0 }]);
//   };

//   const removePointLoad = (index) => {
//     setPointLoads(pointLoads.filter((_, i) => i !== index));
//   };

//   const addDistributedLoad = () => {
//     setdistributedload([...distributedload, { start: 1, end: 3, distributedloadmagnitude: 0 }]);
//   };

//   const removeDistributedLoad = (index) => {
//     setdistributedload(distributedload.filter((_, i) => i !== index));
//   };


//   const addVaryingdistributedload = () => {
//     setVaryingdistributedload([...Varyingdistributedload, { start: 0, end: 0, startMagnitude: 0, endMagnitude: 0 }]);
//   };

//   const removeVaryingdistributedload = (index) => {
//     setVaryingdistributedload(Varyingdistributedload.filter((_, i) => i !== index));
//   };
//   const addSupport = () => {
//     setSupports([...Supports, { position: 5, type: 'fixed' }]);
//   };

//   const removeSupport = (index) => {
//     setSupports(Supports.filter((_, i) => i !== index));
//   };

//   const handleSupportChange = (index, key, value) => {
//     const newSupports = [...Supports];
//     newSupports[index][key] = key === 'position' ? parseFloat(value) : value;
//     setSupports(newSupports);
//   };

//   const handleLoadChange = (key, value) => {
//     setVaryingdistributedload({
//       ...Varyingdistributedload,
//       [key]: parseFloat(value),
//     });
//   };

//   const handlePointLoadChange = (index, key, value) => {
//     const newPointLoads = [...pointLoads];
//     newPointLoads[index][key] = parseFloat(value);
//     setPointLoads(newPointLoads);
//   };


//   const renderSupportsAndLoads = () => {
//     const renderSupports = () => (
//       Supports.map((support, index) => (
//         <g key={index}>
//           <rect
//             x={(support.position / Length) * 500}
//             y={50}
//             width={10}
//             height={20}
//             fill={support.type === 'fixed' ? `'<PinnedSupport></PinnedSupport>'` : support.type === 'pinned' ? 'green' : 'yellow'}
//           />
//           <text x={(support.position / Length) * 500 - 10} y={80} fill="white">
//             {support.type.charAt(0).toUpperCase()}
//           </text>
//         </g>
//       ))
//     );

//     const renderLoad = () => {
//       const startX = (Varyingdistributedload.start / Length) * 500;
//       const endX = (Varyingdistributedload.end / Length) * 500;
//       const gradientId = 'gradient';
//       return (
//         <>
//           <line
//             x1={startX}
//             y1={20}
//             x2={endX}
//             y2={20}
//             stroke={({ gradientId })}
//             strokeWidth={10}
//           />
//           <text x={(startX + endX) / 2 - 50} y={-20} fill="#fff">Distributed Load 1</text>
//           <text x={startX - 30} y={0} fill="#fff">U: {Varyingdistributedload.valueStart}</text>
//           <text x={endX + 10} y={0} fill="#fff">U: {Varyingdistributedload.valueEnd}</text>
//         </>
//       );
//     };

//     const renderPointLoads = () => (
//       pointLoads.map((pointLoad, index) => (
//         <g key={index}>
//           <circle
//             cx={(pointLoad.position / Length) * 500}
//             cy={50}
//             r={5}
//             fill="red"
//           />
//           <text x={(pointLoad.position / Length) * 500 - 50} y={20} fill="#fff">
//             Point Load {pointLoad.magnitude}N
//           </text>
//         </g>
//       ))
//     );

//     return (
//       <svg width="Length" height="100" style={{
//         width: '90%',
//         overflow: 'visible',
//         position: 'relative',
//         left: '50%',
//         transform: 'translate(-50%,50%)'
//       }}>
//         <defs>
//           <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
//             <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
//             <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
//           </linearGradient>
//         </defs>
//         <line x1="0" y1="50" x2="500" y2="50" stroke="black" strokeWidth="2" />
//         {renderSupports()}
//         {renderLoad()}
//         {renderPointLoads()}
//       </svg>
//     );
//   };






//   const renderSupportsAndLoadsforReactions = () => {
//     const renderSupports = () => (
//       Supports.map((support, index) => (
//         <g key={index}>
//           <rect
//             x={(support.position / Length) * 500}
//             y={50}
//             width={10}
//             height={20}
//             fill={support.type === 'fixed' ? `'<PinnedSupport></PinnedSupport>'` : support.type === 'pinned' ? 'green' : 'yellow'}
//           />
//           <text x={(support.position / Length) * 500 - 10} y={80} fill="white">
//             {support.type.charAt(0).toUpperCase()}
//           </text>
//         </g>
//       ))
//     );

//     const renderLoad = () => {
//       const startX = (Varyingdistributedload.start / Length) * 500;
//       const endX = (Varyingdistributedload.end / Length) * 500;
//       const gradientId = 'gradient';
//       return (
//         <>
//           <line
//             x1={startX}
//             y1={20}
//             x2={endX}
//             y2={20}
//             stroke={({ gradientId })}
//             strokeWidth={10}
//           />
//           <text x={(startX + endX) / 2 - 50} y={-20} fill="#fff">Distributed Load 1</text>
//           <text x={startX - 30} y={0} fill="#fff">U: {Varyingdistributedload.valueStart}</text>
//           <text x={endX + 10} y={0} fill="#fff">U: {Varyingdistributedload.valueEnd}</text>
//         </>
//       );
//     };

//     const renderPointLoads = () => (
//       pointLoads.map((pointLoad, index) => (
//         <g key={index}>
//           <circle
//             cx={(pointLoad.position / Length) * 500}
//             cy={50}
//             r={5}
//             fill="red"
//           />
//           <text x={(pointLoad.position / Length) * 500 - 50} y={20} fill="#fff">
//             Point Load {pointLoad.magnitude}N
//           </text>
//         </g>
//       ))
//     );

//     return (
//       <svg width="Length" height="100" style={{
//         width: '90%',
//         overflow: 'visible',
//         position: 'relative',
//         left: '50%',
//         transform: 'translate(-50%,50%)'
//       }}>
//         <defs>
//           <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
//             <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
//             <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
//           </linearGradient>
//         </defs>
//         <line x1="0" y1="50" x2="510" y2="50" stroke="black" strokeWidth="2" />
//         {renderSupports()}
//         {renderLoad()}
//         {renderPointLoads()}
//       </svg>
//     );
//   };






//   const renderReactions = () => {
//     const data = [
//       { x: 0, y: 0 },
//       { x: Length, y: 0 },
//     ];
//     return (
//       <>
//         <XYPlot
//           width={600}
//           height={100}
//           className='Beamplot'
//         >
//           <XAxis />
//           <LineSeries
//             data={data}
//             style={{ stroke: 'black', strokeWidth: 2, transform: 'translate(36px, 10px)' }}
//           />
//         </XYPlot>
//       </>
//     );
//   };

//   const RenderSupports = () => {
//     const renderSupports = () => (
//       Supports.map((support, index) => (
//         <g key={index}>
//           <rect
//             x={(support.position / Length) * 550}
//             y={50}
//             width={10}
//             height={20}
//             fill={support.type === 'fixed' ? `'<PinnedSupport></PinnedSupport>'` : support.type === 'pinned' ? 'green' : 'yellow'}
//           />
//         </g>
//       ))
//     );
//     // const data = [
//     //   { x: 0, y: 50 },
//     //   { x: Length, y: 50 },
//     // ];
//     return (
//       <svg width="Length" height="100" style={{
//         width: '90%',
//         overflow: 'visible',
//       }}>
//         <defs>
//           <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
//             <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
//             <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
//           </linearGradient>
//         </defs>
//         <line x1="0" y1="50" x2="550" y2="50" stroke="black" strokeWidth="2" />
//         {renderSupports()}
//       </svg>
//     );
//   };

//   const RenderLoads = () => {

//     const renderPointLoads = () => (
//       pointLoads.map((pointLoad, index) => (
//         <g key={index}>
//           <circle
//             cx={(pointLoad.position / Length) * 500}
//             cy={50}
//             r={5}
//             fill="red"
//           />
//           <text x={(pointLoad.position / Length) * 500 - 50} y={20} fill="#000">
//             Point Load {pointLoad.magnitude}N
//           </text>
//         </g>
//       ))
//     );
//     // const data = [
//     //   { x: 0, y: 50 },
//     //   { x: Length, y: 50 },
//     // ];
//     return (
//       <>

//         <svg width="Length" height="100" style={{
//           width: '90%',
//           overflow: 'visible',
//         }}>
//           <defs>
//             <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
//               <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
//               <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
//             </linearGradient>
//           </defs>
//           <line x1="0" y1="50" x2="550" y2="50" stroke="black" strokeWidth="2" style={{ transform: 'translate(3px, 0px)' }} />
//           {renderPointLoads()}
//         </svg>
//       </>
//     );
//   };



//   const RenderDistributedLoads = () => {

//     const renderLoad = () => {
//       const startX = (Varyingdistributedload.start / Length) * 500;
//       const endX = (Varyingdistributedload.end / Length) * 500;
//       // const gradientId = 'gradient';
//       return (
//         <>
//           <line
//             x1={startX + 5}
//             y1={endX / 15}
//             x2={endX + 50}
//             y2={startX / 5}
//             height={10}
//             stroke="black"
//           />
//           <text x={(startX + endX) / 2 - 50} y={-20} fill="#000">Distributed Load 1</text>
//           <text x={startX - 0} y={30} fill="#000">U: {Varyingdistributedload.valueStart}</text>
//           <text x={endX + 10} y={30} fill="#000">U: {Varyingdistributedload.valueEnd}</text>
//         </>
//       );
//     };
//     return (
//       <>

//         <svg width="Length" height="100" style={{
//           width: '90%',
//           overflow: 'visible',
//         }}>
//           <defs>
//             <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
//               <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
//               <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
//             </linearGradient>
//           </defs>
//           <line x1="0" y1="50" x2="550" y2="50" stroke="black" strokeWidth="2" style={{ transform: 'translate(3px, 0px)' }} />
//           {renderLoad()}
//         </svg>
//       </>
//     );
//   };


//   const calculateReactions = () => {
//     // Placeholder reaction calculations
//     let reactionForces = {};
//     Supports.forEach((support) => {
//       reactionForces[support.position] = 0;
//     });

//     // Calculate reactions for point loads
//     pointLoads.forEach((load) => {
//       const moments = Supports.map((support) => load.magnitude * (load.position - support.position));
//       const totalMoment = moments.reduce((acc, moment) => acc + moment, 0);
//       Supports.forEach((support) => {
//         reactionForces[support.position] += (totalMoment / Length);
//       });
//     });

//     // Calculate reactions for varying distributed loads
//     Varyingdistributedload.forEach((load) => {
//       const w1 = load.startMagnitude;
//       const w2 = load.endMagnitude;
//       const L = load.end - load.start;
//       const totalLoad = (w1 + w2) * L / 2;
//       const centroid = (2 * L / 3) * (2 * w1 + w2) / (w1 + w2);
//       const moments = Supports.map((support) => totalLoad * (centroid - support.position));
//       const totalMoment = moments.reduce((acc, moment) => acc + moment, 0);
//       Supports.forEach((support) => {
//         reactionForces[support.position] += (totalMoment / Length);
//       });
//     });

//     // Calculate reactions for distributed loads
//     distributedload.forEach((load) => {
//       const w = load.distributedloadmagnitude;
//       const L = load.end - load.start;
//       const totalLoad = w * L;
//       const centroid = (load.start + load.end) / 2;
//       const moments = Supports.map((support) => totalLoad * (centroid - support.position));
//       const totalMoment = moments.reduce((acc, moment) => acc + moment, 0);
//       Supports.forEach((support) => {
//         reactionForces[support.position] += (totalMoment / Length);
//       });
//     });

//     setReactions(reactionForces);

//     const dataPoints = Array.from({ Length: 101 }, (_, i) => i * Length / 100);

//     const shearForceData = dataPoints.map((x) => ({ x, y: calculateShearForce(x) }));
//     setShearForceData(shearForceData);

//     const bendingMomentData = dataPoints.map((x) => ({ x, y: calculateBendingMoment(x) }));
//     setBendingMomentData(bendingMomentData);

//     const deflectionData = dataPoints.map((x) => ({ x, y: calculateDeflection(x) }));
//     setDeflectionData(deflectionData);
//   };

//   const calculateShearForce = (x) => {
//     let shear = 0;
//     pointLoads.forEach((load) => {
//       if (x >= load.position) shear += load.magnitude;
//     });
//     Varyingdistributedload.forEach((load) => {
//       if (x >= load.start && x <= load.end) {
//         const w1 = load.startMagnitude;
//         const w2 = load.endMagnitude;
//         const L = load.end - load.start;
//         const a = x - load.start;
//         shear += (w1 + (w2 - w1) * a / L) * a;
//       }
//     });
//     distributedload.forEach((load) => {
//       if (x >= load.start && x <= load.end) {
//         const w = load.distributedloadmagnitude;
//         const a = x - load.start;
//         shear += w * a;
//       }
//     });
//     Supports.forEach((support) => {
//       if (x >= support.position) shear -= reactions[support.position];
//     });
//     return shear;
//   };

//   // distributedload
//   // setdistributedload
//   const calculateBendingMoment = (x) => {
//     let moment = 0;
//     pointLoads.forEach((load) => {
//       if (x >= load.position) moment += load.magnitude * (x - load.position);
//     });
//     Varyingdistributedload.forEach((load) => {
//       if (x >= load.start && x <= load.end) {
//         const w1 = load.startMagnitude;
//         const w2 = load.endMagnitude;
//         const L = load.end - load.start;
//         const a = x - load.start;
//         moment += (w1 + (w2 - w1) * a / L) * a * (a / 2);
//       }
//     });
//     distributedload.forEach((load) => {
//       if (x >= load.start && x <= load.end) {
//         const w = load.distributedloadmagnitude;
//         const a = x - load.start;
//         moment += w * a * (a / 2);
//       }
//     });
//     Supports.forEach((support) => {
//       if (x >= support.position) moment -= reactions[support.position] * (x - support.position);
//     });
//     return moment;
//   };

//   // const calculateBendingMomentForDeflection = (x) => {
//   //   let moment = 0;
//   //   pointLoads.forEach(pointLoads => {
//   //     if (x >= pointLoads.position) moment -= pointLoads.magnitude * (x - pointLoads.position);
//   //   });
//   //   distributedLoads.forEach(load => {
//   //     if (x >= load.startMagnitude) {
//   //       const a = Math.min(x, load.endMagnitude) - load.startMagnitude;
//   //       moment -= load.startMagnitude * a * (a / 2);
//   //     }
//   //   });
//   //   return moment;
//   // };


//   if (MetricOrImperial === 'option1') {
//     var I = MetricIUnits;
//   } else if (MetricOrImperial === 'option2') {
//     var I = ImperialIunits;
//   }

//   const calculateDeflection = (x) => {
//     const EI = (100 * Length) / (3 * E * I);
//     let deflection = 0;
//     let dx = 0.1;

//     for (let i = 0; i < x; i += dx) {
//       deflection += (1 / EI) * calculateBendingMoment(i) * dx;
//     }

//     return deflection;
//   };

//   // const dataPoints = Array.from({ Length: 101 }, (_, i) => i * Length / 100);






//   const data = [
//     { x: 0, y: 50 },
//     { x: Length, y: 50 },
//   ];

//   // const reactionForce = pointLoads[0].magnitude;
//   // const momentForce = -pointLoads[0].magnitude * (pointLoads[0].position - (Supports[1].position));


//   // shear force
//   // const shearForce = dataPoints.map(x => ({ x, y: calculateShearForce(x) }))
//   // const shear = dataPoints.map(x => ({ y: calculateShearForce(x) }))
//   return (
//     <>
//       <div className='Background-Black'></div>
//       <div className='ME-navbar-links'>
//         <ul className='ME-navbar-links-ul'>
//           <li className='ME-navbar-links-li'>
//             <Link to='/PadEyecalculatorPage'>
//               Pad eye
//             </Link>
//           </li>
//           <li className='ME-navbar-links-li'>
//             <Link to='/Singlepage'>
//               Beam Properties
//             </Link>
//           </li>
//           <li className='ME-navbar-links-li'>
//             <Link to='/BeamDeflectionCalculator'>
//               Beam Deflection
//             </Link>
//           </li>
//           <li className='ME-navbar-links-li'>
//             <Link to='/AboutUs'>
//               About US
//             </Link>
//           </li>
//         </ul>
//       </div>
//       <section className='background-white'>
//         <div className='calculator-first-section-div'>
//           <div className='calculator-first-section-imgdiv' style={{ height: '80vh', overflow: 'hidden' }}>
//             <img className='calculator-first-section-imgdiv-img' src={Backgroundimg} alt='' style={{ transform: 'translate(3vw, -5vw)' }} />
//             <div className='calculator-first-section-imgdiv-shades' />
//           </div>
//           <div className='calculator-first-section-textdiv' style={{ top: '10vw' }}>
//             <h3 className='calculator-first-section-textdiv-h1'>Beam deflection calculator</h3>
//             <p className='calculator-first-section-textdiv-p'>
//               Beam deflection calculator is a powerful tool used by<br />
//               engineers and architects for analyzing the behavior of<br />
//               beams under various loading conditions.
//             </p>
//           </div>
//         </div>

//         <section className='calculator-defination-section '>
//           <br />
//           <hr className='Beam-properties-calculator-hr' />
//           <br />
//           <div className={`calculator-defination-div BeamDeflection`}>
//             <h3 className={`Beam-properties-calculator-heading BeamDeflection ${expanded ? 'expanded' : ''}`}>
//               <span>Beam Deflection Calculator</span>
//             </h3>
//             <div className={`calculator-defination-section-div BeamDeflection ${expanded ? 'expanded' : ''}`}>
//               <h3 className='Beam-properties-calculator-heading'>
//                 <span></span>
//               </h3>
//               <br />
//               <br />
//               <h3 className='calculator-defination-section first-important'>
//                 With the help of the Beam Deflection Calculator, you can easily<br />
//                 examine how different kinds of beams deflect under various loading scenarios.<br />
//                 Beam deflection is an important aspect in structural engineering and construction.

//               </h3>
//               <br />
//               <h3 className='calculator-defination-section second-important'>
//                 It ensures the structural integrity of the beam and helps to preventany potential  deformation or<br /> damage. You can easily find the deflection at any point along the length of the beam<br />  by entering parameters like the beam's material, dimensions, and applied loads.

//               </h3>
//               <br />
//               <h3 className='calculator-defination-section third-important'>
//                 By using this calculator you can easily find the beam's reactions,<br />  maximum deflection, bending moment & shear stress.

//               </h3>
//             </div>
//           </div>
//           <br />
//           <br />
//           <br />
//           <br />
//           <br />
//           <hr className='Beam-properties-calculator-hr' />
//           <br />
//           <br />
//           <br />
//           <section className='structure-analysis-calculator-calculator'>
//             <div className='structure-analysis-calculator-calculator-left'>
//               <br /><br />
//               <button onClick={() => handleOptionChange("option1")}>Length of Beam</button>
//               <button onClick={() => handleOptionChange("option2")}>Young's Modules</button>
//               <button onClick={() => handleOptionChange("option3")}>Cross Section</button>
//               <button onClick={() => handleOptionChange("option4")}>Support</button>
//               <button onClick={() => handleOptionChange("option5")}>Load</button>
//               <br /><br />
//             </div>
//             <div className='structure-analysis-calculator-calculator-center center'>
//               {selectedOption === 'option1' && (
//                 <div style={{
//                 }}>
//                   {renderReactions()}
//                 </div>
//               )}
//               {selectedOption === 'option2' && (
//                 <div></div>

//               )}
//               {selectedOption === 'option3' && (
//                 <div></div>

//               )}
//               {selectedOption === 'option4' && (
//                 <div style={{
//                   // top: '50%',
//                   // left: '50%',
//                   // position: 'absolute',
//                   transform: 'translate(3%,0%)'
//                 }}>
//                   {RenderSupports()}
//                   <XYPlot className='SupportImggraph'
//                     width={600}
//                     height={100}
//                   >
//                     <XAxis />
//                     <LineSeries
//                       data={data}
//                       style={{ stroke: 'black', strokeWidth: 0 }}
//                     />
//                   </XYPlot>
//                 </div>

//               )}
//               {selectedOption === 'option5' && (
//                 <div style={{
//                   // top: '50%',
//                   // left: '50%',
//                   // position: 'absolute',
//                   transform: 'translate(3%,0%)'
//                 }}>
//                   {RenderLoads()}
//                   <XYPlot className='SupportImggraph'
//                     width={600}
//                     height={100}
//                   >
//                     <XAxis />
//                     <LineSeries
//                       data={data}
//                       style={{ stroke: 'black', strokeWidth: 0 }}
//                     />
//                   </XYPlot>
//                   <br />
//                   <br />
//                   <br />
//                   {RenderDistributedLoads()}
//                   <XYPlot className='SupportImggraph'
//                     width={600}
//                     height={100}
//                   >
//                     <XAxis />
//                     <LineSeries
//                       data={data}
//                       style={{ stroke: 'black', strokeWidth: 0 }}
//                     />
//                   </XYPlot>
//                 </div>

//               )}
//             </div>
//             <div className='structure-analysis-calculator-calculator-right'>
//               <br />
//               {selectedOption === 'option1' && (
//                 <>
//                   <LengthOfBeam
//                     MetricOrImperial={MetricOrImperial}
//                     MetricOrImperialBtn={toggleMetricOrImperial}

//                     MetricBeamlengthvalue={metriclength}
//                     MetricLengthOfBeamunits={MetricUnits}
//                     MetricLengthOfBeamSelectedUnit={metriclengthSelectedUnit}
//                     MetrichandleLengthOfBeamChange={((e) => handleMetricInputChange(e.target.value))}
//                     MetrichandleLengthOfBeamUnitchange={((e) => handleMetricUnitChange(e.target.value))}
//                     MetricBeamLengthoptions={
//                       LengthOfBeamunits.map((unit) => (
//                         <option key={unit} value={unit}>
//                           {unit}
//                         </option>
//                       ))
//                     }


//                     ImperialBeamlengthvalue={Imperiallength}
//                     ImperialLengthOfBeamunits={ImperialLengthunits}
//                     ImperialLengthOfBeamSelectedUnit={ImperiallengthSelectedUnit}
//                     ImperialhandleLengthOfBeamChange={((e) => handleImperialInputChange(e.target.value))}
//                     ImperialhandleLengthOfBeamUnitchange={((e) => handleImperialUnitChange(e.target.value))}
//                     ImperialBeamLengthoptions={
//                       ImperialLengthunits.map((unit) => (
//                         <option key={unit} value={unit}>
//                           {unit}
//                         </option>
//                       ))
//                     }
//                     // BeamlengthonChange={(e) => setLength(parseFloat(e.target.value))}
//                     // onChange={handleCrossSectionShapeChange}

//                   />
//                   <SolutionsOfBeamDeflaction

//                     ShearForce={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={shearForceData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     DendingMoment={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={bendingMomentData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     Deflaction={
//                       <ResponsiveContainer>
//                         <LineChart width={500} height={300} data={deflectionData}>
//                           <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                           <XAxis dataKey="x" />
//                           <YAxis />
//                           <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                           <Tooltip />
//                         </LineChart>
//                       </ResponsiveContainer>}

//                     graphText={`${unitText}`}
//                     graphText2={`${unitText2}`}

//                     Reaction={renderSupportsAndLoadsforReactions()}
//                     supports={Supports}
//                     // DataofShearStressPlot={<Line type="monotone" dataKey={shear} stroke="#8884d8" />}
//                     // // DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
//                     // DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
//                     // DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
//                     // reactions={(e) => calculateReactions(e.target.value)}
//                     DataofReactionPlot={
//                       <LineSeries
//                         data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
//                         style={{ strokeWidth: 0 }}
//                       />
//                     }
//                     NumberlineofReactionplot={
//                       <LineSeries
//                         data={data}
//                         style={{ stroke: 'black', strokeWidth: 0 }}
//                       />
//                     }
//                     // LabelSeriesOfReactionForce={
//                     //   <LabelSeries
//                     //     data={[
//                     //       { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
//                     //       { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
//                     //     ]}
//                     //     style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
//                     //   />
//                     // }
//                     // CrossSectionShape={CrossSectionShape}

//                     togglefunction={isActive3}
//                     toggleBreaks={toggleClass3}
//                   />
//                 </>
//               )}
//               {selectedOption === 'option2' && (
//                 <>
//                   <YoungsModules
//                     MetricOrImperial={MetricOrImperial}
//                     MetricOrImperialBtn={toggleMetricOrImperial}

//                     YoungsModulesValue={E}
//                     YoungsModulesSelectedValue={selectedUnit}
//                     YoungsModulesValueOnchange={(e) => handleInputChange(e.target.value)}
//                     YoungsModulesSelectedValueOnchange={(e) => handleUnitChange(e.target.value)}
//                     YoungsModulesOptions={
//                       units.map((unit) => (
//                         <option key={unit} value={unit}>
//                           {unit}
//                         </option>
//                       ))}


//                     ImperialE={ImperialE}
//                     ImperialselectedUnit={ImperialselectedUnit}
//                     handleImperialEInputChange={(e) => handleImperialEInputChange(e.target.value)}
//                     handleImperialEUnitChange={(e) => handleImperialEUnitChange(e.target.value)}
//                     ImperialYoungsModulesOptions={
//                       Imperialunits.map((unit) => (
//                         <option key={unit} value={unit}>
//                           {unit}
//                         </option>
//                       ))}
//                   />
//                   <SolutionsOfBeamDeflaction
//                     ShearForce={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={shearForceData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     DendingMoment={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={bendingMomentData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     Deflaction={
//                       <ResponsiveContainer>
//                         <LineChart width={500} height={300} data={deflectionData}>
//                           <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                           <XAxis dataKey="x" />
//                           <YAxis />
//                           <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                           <Tooltip />
//                         </LineChart>
//                       </ResponsiveContainer>}


//                     graphText={`${unitText}`}
//                     graphText2={`${unitText2}`}

//                     Reaction={renderSupportsAndLoadsforReactions()}
//                     supports={Supports}
//                     // DataofShearStressPlot={<Line type="monotone" dataKey={shear} stroke="#8884d8" />}
//                     // // DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
//                     // DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
//                     // DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
//                     // reactions={(e) => calculateReactions(e.target.value)}
//                     DataofReactionPlot={
//                       <LineSeries
//                         data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
//                         style={{ strokeWidth: 0 }}
//                       />
//                     }
//                     NumberlineofReactionplot={
//                       <LineSeries
//                         data={data}
//                         style={{ stroke: 'black', strokeWidth: 0 }}
//                       />
//                     }
//                     // LabelSeriesOfReactionForce={
//                     //   <LabelSeries
//                     //     data={[
//                     //       { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
//                     //       { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
//                     //     ]}
//                     //     style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
//                     //   />
//                     // }

//                     // CrossSectionShape={CrossSectionShape}

//                     togglefunction={isActive3}
//                     toggleBreaks={toggleClass3}
//                   />
//                 </>
//               )}
//               {selectedOption === 'option3' && (
//                 <>
//                   <CrossSection
//                     MetricOrImperial={MetricOrImperial}
//                     MetricOrImperialBtn={toggleMetricOrImperial}
//                     // A={A}
//                     // AOnchange={(e) => handleAChange(e.target.value)}
//                     MetricAUnits={MetricAUnits}
//                     metricAInputValue={metricAInputValue}
//                     metricASelectedUnit={metricASelectedUnit}
//                     handleMetricAInputChange={(e) => handleMetricAInputChange(e.target.value)}
//                     handleMetricAUnitChange={(e) => handleMetricAUnitChange(e.target.value)}


//                     ImperialAunits={ImperialAunits}
//                     ImperialAinputValue={ImperialAinputValue}
//                     ImperialAselectedUnit={ImperialAselectedUnit}
//                     handleImperialAInputChange={(e) => handleImperialAInputChange(e.target.value)}
//                     handleImperialAUnitChange={(e) => handleImperialAUnitChange(e.target.value)}


//                     // I={I}
//                     // IOnchange={(e) => handleIChange(e.target.value)}


//                     MetricIUnits={MetricIUnits}
//                     metricIInputValue={metricIInputValue}
//                     metricISelectedUnit={metricISelectedUnit}
//                     handleMetricIInputChange={(e) => handleMetricIInputChange(e.target.value)}
//                     handleMetricIUnitChange={(e) => handleMetricIUnitChange(e.target.value)}


//                     ImperialIunits={ImperialIunits}
//                     ImperialIinputValue={ImperialIinputValue}
//                     ImperialIselectedUnit={ImperialIselectedUnit}
//                     handleImperialIInputChange={(e) => handleImperialIInputChange(e.target.value)}
//                     handleImperialIUnitChange={(e) => handleImperialIUnitChange(e.target.value)}
//                   />
//                   <SolutionsOfBeamDeflaction
//                     Sheardata={shearForceData}
//                     bendingMomentDatadata={bendingMomentData}
//                     Deflectiondata={deflectionData}


//                     graphText={`${unitText}`}
//                     graphText2={`${unitText2}`}

//                     Reaction={renderSupportsAndLoadsforReactions()}
//                     supports={Supports}
//                     // DataofShearStressPlot={<Line type="monotone" dataKey={shear} stroke="#8884d8" />}
//                     // // DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
//                     // DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
//                     // DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
//                     // reactions={(e) => calculateReactions(e.target.value)}
//                     DataofReactionPlot={
//                       <LineSeries
//                         data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
//                         style={{ strokeWidth: 0 }}
//                       />
//                     }
//                     NumberlineofReactionplot={
//                       <LineSeries
//                         data={data}
//                         style={{ stroke: 'black', strokeWidth: 0 }}
//                       />
//                     }
//                     // LabelSeriesOfReactionForce={
//                     //   <LabelSeries
//                     //     data={[
//                     //       { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
//                     //       { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
//                     //     ]}
//                     //     style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
//                     //   />
//                     // }
// // CrossSectionShape={CrossSectionShape}

//                     togglefunction={isActive3}
//                     toggleBreaks={toggleClass3}
//                   />
//                 </>
//               )}
//               {selectedOption === 'option4' && (
//                 <>
//                   <SupportPage

//                     addSupport={<button className='BeamLinkBtntoBeamProperies-Addsupport' onClick={addSupport}>Add Support</button>}
//                     Supports={Supports.map((support, index) => (
//                       <div key={index} style={{ width: '100%', display: 'flex', justifyContent: 'space-evenly', alignItems: 'center' }}>
//                         <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
//                           <br />
//                           <div className='input-and-select-div' style={{ justifyContent: 'center' }}>
//                             <select
//                               value={support.type}
//                               onChange={(e) => {
//                                 const newSupports = [...Supports];
//                                 newSupports[index].type = e.target.value;
//                                 setSupports(newSupports);
//                               }}
//                               style={{
//                                 outline: 'none',
//                                 border: 'none'
//                               }}
//                             >
//                               <option value="pinned">Pinned</option>
//                               <option value="roller">Roller</option>
//                               <option value="fixed">Fixed</option>
//                             </select>
//                           </div>
//                         </div>
//                         <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
//                           <br />
//                           <div className='input-and-select-div' style={{ justifyContent: 'center' }}>
//                             <input
//                               style={{
//                                 border: 'none',
//                                 float: 'right',
//                                 textAlign: 'center',
//                                 width: '70%',
//                               }}
//                               type="number"
//                               value={support.position}
//                               onChange={(e) => {
//                                 const newSupports = [...Supports];
//                                 newSupports[index].position = Number(e.target.value);
//                                 setSupports(newSupports);
//                               }}
//                               max={Length}
//                             />
//                           </div>
//                         </div>
//                         <MdDelete style={{ color: 'white', fontSize: '1.5vw' }} onClick={() => removeSupport(index)}>..</MdDelete>
//                       </div>
//                     ))}
//                   />
//                   <SolutionsOfBeamDeflaction
//                     ShearForce={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={shearForceData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     DendingMoment={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={bendingMomentData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     Deflaction={
//                       <ResponsiveContainer>
//                         <LineChart width={500} height={300} data={deflectionData}>
//                           <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                           <XAxis dataKey="x" />
//                           <YAxis />
//                           <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                           <Tooltip />
//                         </LineChart>
//                       </ResponsiveContainer>}


//                     graphText={`${unitText}`}
//                     graphText2={`${unitText2}`}

//                     Reaction={renderSupportsAndLoadsforReactions()}
//                     supports={Supports}
//                     // DataofShearStressPlot={<Line type="monotone" dataKey={shear} stroke="#8884d8" />}
//                     // // DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
//                     // DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
//                     // DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
//                     // reactions={(e) => calculateReactions(e.target.value)}
//                     DataofReactionPlot={
//                       <LineSeries
//                         data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
//                         style={{ strokeWidth: 0 }}
//                       />
//                     }
//                     NumberlineofReactionplot={
//                       <LineSeries
//                         data={data}
//                         style={{ stroke: 'black', strokeWidth: 0 }}
//                       />
//                     }
//                     // LabelSeriesOfReactionForce={
//                     //   <LabelSeries
//                     //     data={[
//                     //       { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
//                     //       { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
//                     //     ]}
//                     //     style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
//                     //   />
//                     // }
// // CrossSectionShape={CrossSectionShape}

//                     togglefunction={isActive3}
//                     toggleBreaks={toggleClass3}
//                   />
//                 </>
//               )}
//               {selectedOption === 'option5' && (
//                 <>
//                   <LoadPage


//                     max={Length}
//                     AddDistributedLoad={
//                       <button className="BeamLinkBtntoBeamProperies-AddLoad BeamLinkBtntoBeamProperies" onClick={addDistributedLoad}>Add +</button>
//                     }
//                     DistributedLoadStart={Varyingdistributedload.start}
//                     DistributedLoadStartOnchange={(e) => handleLoadChange('start', e.target.value)}
//                     DistributedLoadend={Varyingdistributedload.end}
//                     DistributedLoadendOnchange={(e) => handleLoadChange('end', e.target.value)}
//                     DistributedLoadvalueStart={Varyingdistributedload.startMagnitude}
//                     DistributedLoadvalueStartOnchange={(e) => handleLoadChange('startMagnitude', e.target.value)}
//                     DistributedLoadvalueEnd={Varyingdistributedload.endMagnitude}
//                     DistributedLoadvalueEndOnchange={(e) => handleLoadChange('endMagnitude', e.target.value)}

//                     AddPointLoad={
//                       <button className='BeamLinkBtntoBeamProperies-AddLoad BeamLinkBtntoBeamProperies' onClick={addPointLoad}>Add +</button>}
//                     PointLoad={
//                       pointLoads.map((load, index) => (
//                         <>
//                           <h3 style={{ marginBottom: '10px' }}>Point Load {index + 1}</h3>
//                           <div style={{ width: '100%', display: 'flex', justifyContent: 'space-evenly', alignItems: 'center' }}>
//                             <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
//                               <div className='input-and-select-div'
//                                 style={{ justifyContent: 'center' }}
//                               >
//                                 <input
//                                   style={{
//                                     border: 'none',
//                                     textAlign: 'center',
//                                     width: '100%',
//                                     display: 'flex',
//                                     justifyContent: 'center',
//                                     alignItems: 'center'

//                                   }}
//                                   type='number'
//                                   value={load.position}
//                                   onChange={(e) => handlePointLoadChange(index, 'position', e.target.value)}
//                                   step='1'
//                                   min='0'
//                                   max={Length}
//                                   placeholder='Position'
//                                 />

//                               </div>
//                             </div>
//                             <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
//                               <div className='input-and-select-div'
//                                 style={{ justifyContent: 'center', width: '100%' }}
//                               >
//                                 <input
//                                   style={{
//                                     border: 'none',
//                                     textAlign: 'center', width: '100%',
//                                     display: 'flex',
//                                     justifyContent: 'center',
//                                     alignItems: 'center'

//                                   }}
//                                   type='number'
//                                   value={load.magnitude}
//                                   onChange={(e) => handlePointLoadChange(index, 'magnitude', e.target.value)}
//                                   placeholder='Magnitude'
//                                 />


//                               </div>
//                             </div>
//                             <MdDelete style={{ color: 'white', fontSize: '1.5vw' }} onClick={() => removePointLoad(index)}>..</MdDelete>
//                           </div>
//                         </>
//                       ))}


//                     MetricOrImperial={MetricOrImperial}
//                     MetricOrImperialBtn={toggleMetricOrImperial}

//                     MetricBeamlengthvalue={metriclength}
//                     MetricLengthOfBeamunits={MetricUnits}
//                     MetricLengthOfBeamSelectedUnit={metriclengthSelectedUnit}
//                     MetrichandleLengthOfBeamChange={((e) => handleMetricInputChange(e.target.value))}
//                     MetrichandleLengthOfBeamUnitchange={((e) => handleMetricUnitChange(e.target.value))}
//                     MetricBeamLengthoptions={
//                       LengthOfBeamunits.map((unit) => (
//                         <option key={unit} value={unit}>
//                           {unit}
//                         </option>
//                       ))
//                     }




//                     ImperialBeamlengthvalue={Imperiallength}
//                     ImperialLengthOfBeamunits={ImperialLengthunits}
//                     ImperialLengthOfBeamSelectedUnit={ImperiallengthSelectedUnit}
//                     ImperialhandleLengthOfBeamChange={((e) => handleImperialInputChange(e.target.value))}
//                     ImperialhandleLengthOfBeamUnitchange={((e) => handleImperialUnitChange(e.target.value))}
//                     ImperialBeamLengthoptions={
//                       ImperialLengthunits.map((unit) => (
//                         <option key={unit} value={unit}>
//                           {unit}
//                         </option>
//                       ))
//                     }

//                   // Beamlengthvalue={length}
//                   // LengthOfBeamunits={LengthOfBeamunits}
//                   // LengthOfBeamSelectedUnit={LengthOfBeamSelectedUnit}
//                   // handleLengthOfBeamChange={((e) => handleLengthOfBeamChange(e.target.value))}
//                   // handleLengthOfBeamUnitchange={((e) => handleLengthOfBeamUnitchange(e.target.value))}
//                   // BeamLengthoptions={
//                   // LengthOfBeamunits.map((unit) => (
//                   // <option key={unit} value={unit}>
//                   // {/* {unit} */}
//                   // {/* </option> */}
//                   // ))
//                   // }
//                   />
//                   <SolutionsOfBeamDeflaction
//                     LoadBendingDeflectionclasses={'LoadBendingDeflectionclasses'}
//                     ShearForce={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={shearForceData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     DendingMoment={<ResponsiveContainer>
//                       <LineChart width={500} height={300} data={bendingMomentData}>
//                         <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                         <XAxis dataKey="x" />
//                         <YAxis />
//                         <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                         <Tooltip />
//                       </LineChart>
//                     </ResponsiveContainer>}
//                     Deflaction={
//                       <ResponsiveContainer>
//                         <LineChart width={500} height={300} data={deflectionData}>
//                           <Line type="monotone" dataKey="y" stroke="#8884d8" dot={false} />
//                           <XAxis dataKey="x" />
//                           <YAxis />
//                           <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
//                           <Tooltip />
//                         </LineChart>
//                       </ResponsiveContainer>}


//                     graphText={`${unitText}`}
//                     graphText2={`${unitText2}`}

//                     Reaction={renderSupportsAndLoadsforReactions()}
//                     supports={Supports}
//                     // DataofShearStressPlot={<Line type="monotone" dataKey={shear} stroke="#8884d8" />}
//                     // // DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
//                     // DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
//                     // DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
//                     // reactions={(e) => calculateReactions(e.target.value)}
//                     DataofReactionPlot={
//                       <LineSeries
//                         data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
//                         style={{ strokeWidth: 0 }}
//                       />
//                     }
//                     NumberlineofReactionplot={
//                       <LineSeries
//                         data={data}
//                         style={{ stroke: 'black', strokeWidth: 0 }}
//                       />
//                     }
//                     // LabelSeriesOfReactionForce={
//                     //   <LabelSeries
//                     //     data={[
//                     //       { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
//                     //       { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
//                     //     ]}
//                     //     style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
//                     //   />
//                     // }
// // CrossSectionShape={CrossSectionShape}

//                     togglefunction={isActive3}
//                     toggleBreaks={toggleClass3}
//                   />
//                 </>
//               )}
//             </div>
//           </section>

//           <br />
//           <br />
//           <br />
//           <br />
//           <div className={isActive3 ? ' Beamdeflaction height110 ' : ' Beamdeflaction height110 height0 '} ></div>
//           <hr className='Beam-properties-calculator-hr' />
//           <br />
//           <br />
//           <br />
//           <br />
//           <BeamDeflection />
//           <br />
//           <br />
//           <br />
//           <hr className='Beam-properties-calculator-hr' />
//           <br />
//           <br />
//           <br />
//           <br />
//           <br />
//           <Load />
//           <hr className='Beam-properties-calculator-hr' />
//           <br />
//           <br />
//           <br />
//           <br />
//           <br />
//           <br />
//         </section>
//       </section>
//       <footer className='footer'></footer>
//     </>
//   )
// }


// Modify this code and remove all the bugs but make sure the functions work properly


import React, { useState, useEffect } from 'react';
import { XYPlot, XAxis, LabelSeries, LineSeries, } from 'react-vis';
import { Line } from 'recharts';
import 'react-vis/dist/style.css';
import { MdDelete } from "react-icons/md";
import Backgroundimg from '../image folder/Deflaction2.jpg'
import BeamDeflection from './Beam Deflection dropdown/Beam Deflection'
import Load from './Beam Deflection dropdown/Load'
import './Beam Deflection dropdown/Beam Deflection.css'
import { Link } from 'react-router-dom'
import LengthOfBeam from './BeamDeflactionCalculator/LengthOfBeam'
import YoungsModules from './BeamDeflactionCalculator/YoungsModules'
import CrossSection from './BeamDeflactionCalculator/CrossSection'
import SupportPage from './BeamDeflactionCalculator/Support'
import LoadPage from './BeamDeflactionCalculator/Load'
import SolutionsOfBeamDeflaction from './SolutionsOfBeamDeflaction';
export default function BeamDeflectionCalculatorTest() {
  const [selectedOption, setSelectedOption] = useState("option1");
  const handleOptionChange = (option) => {
    setSelectedOption(option)
  }

  const [isActive3, setIsActive3] = useState(true);

  const toggleClass3 = () => {
    setIsActive3(previsActive3 => !previsActive3);

  };
  const [MetricOrImperial, setMetricOrImperial] = useState("option1");

  const toggleMetricOrImperial = (option) => {
    setMetricOrImperial(option);

  };
  const [expanded, setExpanded] = useState(false);
  const [Sectionthird, setSectionthird] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 100 && !expanded) {
        setExpanded(true);
      }
      if (window.scrollY > 350 && !Sectionthird) {
        setSectionthird(true);
      }
    };

    window.addEventListener('scroll', handleScroll);

    return () => {
      window.removeEventListener('scroll', handleScroll);
    };
  }, [expanded, Sectionthird]);





  // calculator functions
  const MetricUnits = ['mm', 'm'];
  const MetricConversionFactors = {
    mm: [1, 0.001],
    m: [1000, 1],
  };
  const [metriclength, setmetriclength] = useState('');
  const [metriclengthSelectedUnit, setmetriclengthSelectedUnit] = useState('mm');
  const [internalMetricValue, setInternalMetricValue] = useState(0); // Always in mm
  const handleMetricInputChange = (value) => {
    setmetriclength(value);
    const factor = MetricConversionFactors[metriclengthSelectedUnit][0];
    setInternalMetricValue(parseFloat(value) * factor);
  };

  const handleMetricUnitChange = (unit) => {
    const newMetricValueInMM = parseFloat(metriclength) * MetricConversionFactors[metriclengthSelectedUnit][0];
    const convertedValue = newMetricValueInMM / MetricConversionFactors[unit][0];
    setmetriclengthSelectedUnit(unit);
    setmetriclength(convertedValue.toFixed(5));



    const lengthfactor = MetricConversionFactors[metriclengthSelectedUnit][MetricUnits.indexOf(unit)];
    setLength((parseFloat(metriclength) * lengthfactor).toFixed(2));
    setLengthOfBeamSelectedUnit(unit);

    switch (unit) {
      case 'mm':
        setUnitText('N');
        setUnitText2('mm');
        break;
      case 'm':
        setUnitText('KN');
        setUnitText2('m');
        break;
      default:
        setUnitText('N');
        setUnitText2('mm');
        break;
    }
  };


  const ImperialLengthunits = ['in'];
  const ImperialFactors = {
    in: [1],
  };

  const [Imperiallength, setImperiallength] = useState('');
  const [ImperiallengthSelectedUnit, setImperiallengthSelectedUnit] = useState('in');

  const handleImperialInputChange = (value) => {
    setImperiallength(value);
  };

  const handleImperialUnitChange = (unit) => {
    setImperiallengthSelectedUnit(unit);
    const Imperialfactor = ImperialFactors[unit][ImperialLengthunits.indexOf(ImperiallengthSelectedUnit)];
    setImperiallength((parseFloat(Imperiallength) / Imperialfactor).toFixed(3));


    const lengthfactor = ImperialFactors[ImperiallengthSelectedUnit][ImperialLengthunits.indexOf(unit)];
    setLength((parseFloat(Imperiallength) * lengthfactor).toFixed(2));
    setLengthOfBeamSelectedUnit(unit);

    switch (unit) {
      case 'in':
        setUnitText('lb');
        setUnitText2('in');
        break;
      default:
        setUnitText('lb');
        setUnitText2('in');
        break;
    }
  };

  const [unitText, setUnitText] = useState('N');
  const [unitText2, setUnitText2] = useState('mm');

  const units = ['Mpa', 'Pa'];
  const conversionFactors = {
    Mpa: [1, 1e6],
    Pa: [1e-6, 1],
  };
  const [E, setE] = useState('');
  const [selectedUnit, setSelectedUnit] = useState('Mpa');
  const [EInMpa, setEInMpa] = useState('');

  const handleInputChange = (value) => {
    setE(value);
    const factor = conversionFactors[selectedUnit][0];
    setEInMpa(parseFloat(value) * factor);
  };

  const handleUnitChange = (unit) => {
    let newValue = parseFloat(E);
    if (unit === 'Pa' && selectedUnit === 'Mpa') {
      newValue *= 1e6;
      if (newValue >= 1000) {
        newValue = newValue.toExponential(3);
      }
    } else if (unit === 'Mpa' && selectedUnit === 'Pa') {
      newValue /= 1e6;
    }
    setSelectedUnit(unit);
    setE(newValue);
  };

  const Imperialunits = ['Psi'];
  const ImperialconversionFactors = {
    Psi: [1],
  };
  const [ImperialE, setImperialE] = useState('');
  const [ImperialselectedUnit, setImperialSelectedUnit] = useState('Psi');

  const handleImperialEInputChange = (value) => {
    setImperialE(value);
  };

  const handleImperialEUnitChange = (unit) => {
    setImperialSelectedUnit(unit);
  };

  if (MetricOrImperial === "option1") {
    var Length = internalMetricValue;
    var LengthOfBeamconversionFactors = MetricConversionFactors;
    var LengthOfBeamSelectedUnit = metriclengthSelectedUnit;
    var LengthOfBeamunits = MetricUnits;
    var setLength = setmetriclengthSelectedUnit;
    var setLengthOfBeamSelectedUnit = setmetriclengthSelectedUnit;
    var VarE = EInMpa
  } else if (MetricOrImperial === "option2") {
    var Length = Imperiallength;
    var LengthOfBeamconversionFactors = ImperialFactors;
    var LengthOfBeamSelectedUnit = ImperiallengthSelectedUnit;
    var LengthOfBeamunits = ImperialLengthunits;
    var setLength = setImperiallength;
    var setLengthOfBeamSelectedUnit = setImperiallengthSelectedUnit;
    var VarE = ImperialE

  }

  const [A, setA] = useState(2.759006); // Area in in^2
  const handleAChange = (value) => {
    setA(value);
  };



  const MetricAUnits = ['mm', 'm'];
  const MetricAConversionFactors = {
    mm: [1, 0.001],
    m: [1000, 1],
  };
  const [metricAInputValue, setMetricAInputValue] = useState('');
  const [metricASelectedUnit, setMetricASelectedUnit] = useState('mm');
  const [internalMetricAValue, setInternalMetricAValue] = useState(0); // Always in mm

  const handleMetricAInputChange = (value) => {
    setMetricAInputValue(value);
    const factor = MetricAConversionFactors[metricASelectedUnit][0];
    setInternalMetricAValue(parseFloat(value) * factor);
  };

  const handleMetricAUnitChange = (unit) => {
    const newMetricValueInMM = parseFloat(metricAInputValue) * MetricAConversionFactors[metricASelectedUnit][0];
    const convertedValue = newMetricValueInMM / MetricAConversionFactors[unit][0];
    setMetricASelectedUnit(unit);
    setMetricAInputValue(convertedValue.toFixed(5));
  };



  const ImperialAunits = ['in'];
  const ImperialAFactors = {
    in: [1],
  };

  const [ImperialAinputValue, setImperialAInputValue] = useState('');
  const [ImperialAselectedUnit, setImperialASelectedUnit] = useState('in');

  const handleImperialAInputChange = (value) => {
    setImperialAInputValue(value);
  };

  const handleImperialAUnitChange = (unit) => {
    setImperialASelectedUnit(unit);
    const Imperialfactor = ImperialAFactors[unit][ImperialAunits.indexOf(ImperialAselectedUnit)];
    setImperialAInputValue((parseFloat(ImperialAinputValue) / Imperialfactor).toFixed(3));
  };


  const [I, setI] = useState(16.0007); // Moment of Inertia in in^4
  const handleIChange = (value) => {
    setI(value);
  };

  const MetricIUnits = ['mm', 'm'];
  const MetricIConversionFactors = {
    mm: [1, 0.001],
    m: [1000, 1],
  };
  const [metricIInputValue, setMetricIInputValue] = useState('');
  const [metricISelectedUnit, setMetricISelectedUnit] = useState('mm');
  const [internalMetricIValue, setInternalMetricIValue] = useState(0); // Always in mm

  const handleMetricIInputChange = (value) => {
    setMetricIInputValue(value);
    const factor = MetricIConversionFactors[metricISelectedUnit][0];
    setInternalMetricIValue(parseFloat(value) * factor);
  };

  const handleMetricIUnitChange = (unit) => {
    const newMetricValueInMM = parseFloat(metricIInputValue) * MetricIConversionFactors[metricISelectedUnit][0];
    const convertedValue = newMetricValueInMM / MetricIConversionFactors[unit][0];
    setMetricISelectedUnit(unit);
    setMetricIInputValue(convertedValue.toFixed(5));
  };



  const ImperialIunits = ['in'];
  const ImperialIFactors = {
    in: [1],
  };

  const [ImperialIinputValue, setImperialIInputValue] = useState('');
  const [ImperialIselectedUnit, setImperialISelectedUnit] = useState('in');

  const handleImperialIInputChange = (value) => {
    setImperialIInputValue(value);
  };

  const handleImperialIUnitChange = (unit) => {
    setImperialISelectedUnit(unit);
    const Imperialfactor = ImperialIFactors[unit][ImperialIunits.indexOf(ImperialIselectedUnit)];
    setImperialIInputValue((parseFloat(ImperialIinputValue) / Imperialfactor).toFixed(3));
  };




  const [distributedLoads, setDistributedLoads] = useState([]);
  const [reactions, setReactions] = useState({ RA: 0, RB: 0 });
  const [beam, setBeam] = useState({ length: 10 });
  const [Supports, setSupports] = useState([
    { position: 0, type: 'roller' },
    { position: 2000, type: 'fixed' },
  ]);

  const [pointLoads, setPointLoads] = useState([{ position: 0, magnitude: 0 }]);

  // Varying distributed load
  const [Varyingdistributedload, setVaryingdistributedload] = useState({ start: 0, end: 0, startMagnitude: 0, endMagnitude: 0 });
  // distributed load
  const [distributedload, setdistributedload] = useState({ start: 0, end: 0, Magnitude: 0 });

  useEffect(() => {
    calculateReactions();
  }, [pointLoads, distributedLoads, Supports]);

  const addPointLoad = () => {
    setPointLoads([...pointLoads, { position: 0, magnitude: 0 }]);
  };

  const removePointLoad = (index) => {
    setPointLoads(pointLoads.filter((_, i) => i !== index));
  };

  const addDistributedLoad = () => {
    setDistributedLoads([...distributedLoads, { start: 0, end: 0, startMagnitude: 0, endMagnitude: 0 }]);
  };

  const removeDistributedLoad = (index) => {
    setDistributedLoads(distributedLoads.filter((_, i) => i !== index));
  };

  const addSupport = () => {
    setSupports([...Supports, { type: 'pinned', position: 0 }]);
  };

  const removeSupport = (index) => {
    setSupports(Supports.filter((_, i) => i !== index));
  };

  const handleSupportChange = (index, key, value) => {
    const newSupports = [...Supports];
    newSupports[index][key] = key === 'position' ? parseFloat(value) : value;
    setSupports(newSupports);
  };

  const handleLoadChange = (key, value) => {
    setVaryingdistributedload({
      ...Varyingdistributedload,
      [key]: parseFloat(value),
    });
  };

  const handlePointLoadChange = (index, key, value) => {
    const newPointLoads = [...pointLoads];
    newPointLoads[index][key] = parseFloat(value);
    setPointLoads(newPointLoads);
  };


  const renderSupportsAndLoads = () => {
    const renderSupports = () => (
      Supports.map((support, index) => (
        <g key={index}>
          <rect
            x={(support.position / Length) * 500}
            y={50}
            width={10}
            height={20}
            fill={support.type === 'fixed' ? `'<PinnedSupport></PinnedSupport>'` : support.type === 'pinned' ? 'green' : 'yellow'}
          />
          <text x={(support.position / Length) * 500 - 10} y={80} fill="white">
            {support.type.charAt(0).toUpperCase()}
          </text>
        </g>
      ))
    );

    const renderLoad = () => {
      const startX = (Varyingdistributedload.start / Length) * 500;
      const endX = (Varyingdistributedload.end / Length) * 500;
      const gradientId = 'gradient';
      return (
        <>
          <line
            x1={startX}
            y1={20}
            x2={endX}
            y2={20}
            stroke={({ gradientId })}
            strokeWidth={10}
          />
          <text x={(startX + endX) / 2 - 50} y={-20} fill="#fff">Distributed Load 1</text>
          <text x={startX - 30} y={0} fill="#fff">U: {Varyingdistributedload.valueStart}</text>
          <text x={endX + 10} y={0} fill="#fff">U: {Varyingdistributedload.valueEnd}</text>
        </>
      );
    };

    const renderPointLoads = () => (
      pointLoads.map((pointLoad, index) => (
        <g key={index}>
          <circle
            cx={(pointLoad.position / Length) * 500}
            cy={50}
            r={5}
            fill="red"
          />
          <text x={(pointLoad.position / Length) * 500 - 50} y={20} fill="#fff">
            Point Load {pointLoad.value}N
          </text>
        </g>
      ))
    );

    return (
      <svg width="Length" height="100" style={{
        width: '90%',
        overflow: 'visible',
        position: 'relative',
        left: '50%',
        transform: 'translate(-50%,50%)'
      }}>
        <defs>
          <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
            <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
          </linearGradient>
        </defs>
        <line x1="0" y1="50" x2="500" y2="50" stroke="black" strokeWidth="2" />
        {renderSupports()}
        {renderLoad()}
        {renderPointLoads()}
      </svg>
    );
  };






  const renderSupportsAndLoadsforReactions = () => {
    const renderSupports = () => (
      Supports.map((support, index) => (
        <g key={index}>
          <rect
            x={(support.position / Length) * 500}
            y={50}
            width={10}
            height={20}
            fill={support.type === 'fixed' ? `'<PinnedSupport></PinnedSupport>'` : support.type === 'pinned' ? 'green' : 'yellow'}
          />
          <text x={(support.position / Length) * 500 - 10} y={80} fill="white">
            {support.type.charAt(0).toUpperCase()}
          </text>
        </g>
      ))
    );

    const renderLoad = () => {
      const startX = (Varyingdistributedload.start / Length) * 500;
      const endX = (Varyingdistributedload.end / Length) * 500;
      const gradientId = 'gradient';
      return (
        <>
          <line
            x1={startX}
            y1={20}
            x2={endX}
            y2={20}
            stroke={({ gradientId })}
            strokeWidth={10}
          />
          <text x={(startX + endX) / 2 - 50} y={-20} fill="#fff">Distributed Load 1</text>
          <text x={startX - 30} y={0} fill="#fff">U: {Varyingdistributedload.valueStart}</text>
          <text x={endX + 10} y={0} fill="#fff">U: {Varyingdistributedload.valueEnd}</text>
        </>
      );
    };

    const renderPointLoads = () => (
      pointLoads.map((pointLoad, index) => (
        <g key={index}>
          <circle
            cx={(pointLoad.position / Length) * 500}
            cy={50}
            r={5}
            fill="red"
          />
          <text x={(pointLoad.position / Length) * 500 - 50} y={20} fill="#fff">
            Point Load {pointLoad.value}N
          </text>
        </g>
      ))
    );

    return (
      <svg width="Length" height="100" style={{
        width: '90%',
        overflow: 'visible',
        position: 'relative',
        left: '50%',
        transform: 'translate(-50%,50%)'
      }}>
        <defs>
          <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
            <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
          </linearGradient>
        </defs>
        <line x1="0" y1="50" x2="510" y2="50" stroke="black" strokeWidth="2" />
        {renderSupports()}
        {renderLoad()}
        {renderPointLoads()}
      </svg>
    );
  };






  const renderReactions = () => {
    const data = [
      { x: 0, y: 0 },
      { x: Length, y: 0 },
    ];
    return (
      <>
        <XYPlot
          width={600}
          height={100}
          className='Beamplot'
        >
          <XAxis />
          <LineSeries
            data={data}
            style={{ stroke: 'black', strokeWidth: 2, transform: 'translate(36px, 10px)' }}
          />
        </XYPlot>
      </>
    );
  };

  const RenderSupports = () => {
    const renderSupports = () => (
      Supports.map((support, index) => (
        <g key={index}>
          <rect
            x={(support.position / Length) * 550}
            y={50}
            width={10}
            height={20}
            fill={support.type === 'fixed' ? `'<PinnedSupport></PinnedSupport>'` : support.type === 'pinned' ? 'green' : 'yellow'}
          />
        </g>
      ))
    );
    // const data = [
    //   { x: 0, y: 50 },
    //   { x: Length, y: 50 },
    // ];
    return (
      <svg width="Length" height="100" style={{
        width: '90%',
        overflow: 'visible',
      }}>
        <defs>
          <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
            <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
          </linearGradient>
        </defs>
        <line x1="0" y1="50" x2="550" y2="50" stroke="black" strokeWidth="2" />
        {renderSupports()}
      </svg>
    );
  };

  const RenderLoads = () => {

    const renderPointLoads = () => (
      pointLoads.map((pointLoad, index) => (
        <g key={index}>
          <circle
            cx={(pointLoad.position / Length) * 500}
            cy={50}
            r={5}
            fill="red"
          />
          <text x={(pointLoad.position / Length) * 500 - 50} y={20} fill="#000">
            Point Load {pointLoad.value}N
          </text>
        </g>
      ))
    );
    // const data = [
    //   { x: 0, y: 50 },
    //   { x: Length, y: 50 },
    // ];
    return (
      <>

        <svg width="Length" height="100" style={{
          width: '90%',
          overflow: 'visible',
        }}>
          <defs>
            <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
              <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
            </linearGradient>
          </defs>
          <line x1="0" y1="50" x2="550" y2="50" stroke="black" strokeWidth="2" style={{ transform: 'translate(3px, 0px)' }} />
          {renderPointLoads()}
        </svg>
      </>
    );
  };



  const RenderDistributedLoads = () => {

    const renderLoad = () => {
      const startX = (Varyingdistributedload.start / Length) * 500;
      const endX = (Varyingdistributedload.end / Length) * 500;
      // const gradientId = 'gradient';
      return (
        <>
          <line
            x1={startX + 5}
            y1={endX / 15}
            x2={endX + 50}
            y2={startX / 5}
            height={10}
            stroke="black"
          />
          <text x={(startX + endX) / 2 - 50} y={-20} fill="#000">Distributed Load 1</text>
          <text x={startX - 0} y={30} fill="#000">U: {Varyingdistributedload.valueStart}</text>
          <text x={endX + 10} y={30} fill="#000">U: {Varyingdistributedload.valueEnd}</text>
        </>
      );
    };
    return (
      <>

        <svg width="Length" height="100" style={{
          width: '90%',
          overflow: 'visible',
        }}>
          <defs>
            <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" style={{ stopColor: 'gray', stopOpacity: 1 }} />
              <stop offset="100%" style={{ stopColor: 'black', stopOpacity: 1 }} />
            </linearGradient>
          </defs>
          <line x1="0" y1="50" x2="550" y2="50" stroke="black" strokeWidth="2" style={{ transform: 'translate(3px, 0px)' }} />
          {renderLoad()}
        </svg>
      </>
    );
  };


  const calculateReactions = () => {
    // Simplified reaction calculation
    const leftSupport = Supports.find(support => support.position === Math.min(...Supports.map(s => s.position)));
    const rightSupport = Supports.find(support => support.position === Math.max(...Supports.map(s => s.position)));
    let RA = 0, RB = 0;

    pointLoads.forEach(load => {
      const a = load.position - leftSupport.position;
      const b = rightSupport.position - load.position;
      RA += load.magnitude * (b / Length);
      RB += load.magnitude * (a / Length);
    });

    distributedLoads.forEach(load => {
      const w = (load.startMagnitude + load.endMagnitude) / 2;
      const a = (load.start + load.end) / 2 - leftSupport.position;
      const b = rightSupport.position - (load.start + load.end) / 2;
      RA += w * (b / Length);
      RB += w * (a / Length);
    });

    setReactions({ RA, RB });
  };

  const calculateShearForce = (x) => {
    let shear = 0;
    pointLoads.forEach(pointLoads => {
      if (x >= pointLoads.position) shear += pointLoads.magnitude;
    });
    distributedload.forEach(load => {
      if (x >= load.startMagnitude) shear += load.startMagnitude * (Math.min(x, load.endMagnitude) - load.startMagnitude);
    });
    return shear;
  };
  // distributedload
  // setdistributedload
  const calculateBendingMoment = (x) => {
    let moment = 0;
    pointLoads.forEach(pointLoads => {
      if (x >= pointLoads.position) moment += pointLoads.magnitude * (x - pointLoads.position);
    });
    distributedLoads.forEach(load => {
      if (x >= load.startMagnitude) {
        const a = Math.min(x, load.endMagnitude) - load.startMagnitude;
        moment += load.startMagnitude * a * (a / 2);
      }
    });
    return moment;
  };

  const calculateBendingMomentForDeflection = (x) => {
    let moment = 0;
    pointLoads.forEach(pointLoads => {
      if (x >= pointLoads.position) moment -= pointLoads.magnitude * (x - pointLoads.position);
    });
    distributedLoads.forEach(load => {
      if (x >= load.startMagnitude) {
        const a = Math.min(x, load.endMagnitude) - load.startMagnitude;
        moment -= load.startMagnitude * a * (a / 2);
      }
    });
    return moment;
  };
  const calculateDeflection = (x) => {
    const EI = (100 * Length) / (3 * VarE * I);
    // let EI = (E * I);
    let deflection = 0;

    for (let i = 0; i < x; i += 0.1) {
      deflection += (1 / EI) * calculateBendingMomentForDeflection(i) * 0.1;
    }

    return deflection;
  };

  const dataPoints = Array.from({ Length: 101 }, (_, i) => i * Length / 100);





  const [CrossSectionShape, setCrossSectionShape] = (useState("Square"));

  const handleCrossSectionShapeChange = (newShape) => {
    setCrossSectionShape(newShape)
  }

  const data = [
    { x: 0, y: 50 },
    { x: Length, y: 50 },
  ];

  const reactionForce = pointLoads[0].magnitude;
  const momentForce = -pointLoads[0].magnitude * (pointLoads[0].position - (Supports[1].position));


  // shear force
  const shearForce = dataPoints.map(x => ({ x, y: calculateShearForce(x) }))
  const shear = dataPoints.map(x => ({ y: calculateShearForce(x) }))
  return (
    <>
      <div className='Background-Black'></div>
      <div className='ME-navbar-links'>
        <ul className='ME-navbar-links-ul'>
          <li className='ME-navbar-links-li'>
            <Link to='/PadEyecalculatorPage'>
              Pad eye
            </Link>
          </li>
          <li className='ME-navbar-links-li'>
            <Link to='/Singlepage'>
              Beam Properties
            </Link>
          </li>
          <li className='ME-navbar-links-li'>
            <Link to='/BeamDeflectionCalculator'>
              Beam Deflection
            </Link>
          </li>
          <li className='ME-navbar-links-li'>
            <Link to='/AboutUs'>
              About US
            </Link>
          </li>
        </ul>
      </div>
      <section className='background-white'>
        <div className='calculator-first-section-div'>
          <div className='calculator-first-section-imgdiv' style={{ height: '80vh', overflow: 'hidden' }}>
            <img className='calculator-first-section-imgdiv-img' src={Backgroundimg} alt='' style={{ transform: 'translate(3vw, -5vw)' }} />
            <div className='calculator-first-section-imgdiv-shades' />
          </div>
          <div className='calculator-first-section-textdiv' style={{ top: '10vw' }}>
            <h3 className='calculator-first-section-textdiv-h1'>Beam deflection calculator</h3>
            <p className='calculator-first-section-textdiv-p'>
              Beam deflection calculator is a powerful tool used by<br />
              engineers and architects for analyzing the behavior of<br />
              beams under various loading conditions.
            </p>
          </div>
        </div>

        <section className='calculator-defination-section '>
          <br />
          <hr className='Beam-properties-calculator-hr' />
          <br />
          <div className={`calculator-defination-div BeamDeflection`}>
            <h3 className={`Beam-properties-calculator-heading BeamDeflection ${expanded ? 'expanded' : ''}`}>
              <span>Beam Deflection Calculator</span>
            </h3>
            <div className={`calculator-defination-section-div BeamDeflection ${expanded ? 'expanded' : ''}`}>
              <h3 className='Beam-properties-calculator-heading'>
                <span></span>
              </h3>
              <br />
              <br />
              <h3 className='calculator-defination-section first-important'>
                With the help of the Beam Deflection Calculator, you can easily<br />
                examine how different kinds of beams deflect under various loading scenarios.<br />
                Beam deflection is an important aspect in structural engineering and construction.

              </h3>
              <br />
              <h3 className='calculator-defination-section second-important'>
                It ensures the structural integrity of the beam and helps to preventany potential  deformation or<br /> damage. You can easily find the deflection at any point along the length of the beam<br />  by entering parameters like the beam's material, dimensions, and applied loads.

              </h3>
              <br />
              <h3 className='calculator-defination-section third-important'>
                By using this calculator you can easily find the beam's reactions,<br />  maximum deflection, bending moment & shear stress.

              </h3>
            </div>
          </div>
          <br />
          <br />
          <br />
          <br />
          <br />
          <hr className='Beam-properties-calculator-hr' />
          <br />
          <br />
          <br />
          <section className='structure-analysis-calculator-calculator'>
            <div className='structure-analysis-calculator-calculator-left'>
              <br /><br />
              <button onClick={() => handleOptionChange("option1")}>Length of Beam</button>
              <button onClick={() => handleOptionChange("option2")}>Young's Modules</button>
              <button onClick={() => handleOptionChange("option3")}>Cross Section</button>
              <button onClick={() => handleOptionChange("option4")}>Support</button>
              <button onClick={() => handleOptionChange("option5")}>Load</button>
              <br /><br />
            </div>
            <div className='structure-analysis-calculator-calculator-center center'>
              {selectedOption === 'option1' && (
                <div style={{
                }}>
                  {renderReactions()}
                </div>
              )}
              {selectedOption === 'option2' && (
                <div></div>

              )}
              {selectedOption === 'option3' && (
                <div></div>

              )}
              {selectedOption === 'option4' && (
                <div style={{
                  // top: '50%',
                  // left: '50%',
                  // position: 'absolute',
                  transform: 'translate(3%,0%)'
                }}>
                  {RenderSupports()}
                  <XYPlot className='SupportImggraph'
                    width={600}
                    height={100}
                  >
                    <XAxis />
                    <LineSeries
                      data={data}
                      style={{ stroke: 'black', strokeWidth: 0 }}
                    />
                  </XYPlot>
                </div>

              )}
              {selectedOption === 'option5' && (
                <div style={{
                  // top: '50%',
                  // left: '50%',
                  // position: 'absolute',
                  transform: 'translate(3%,0%)'
                }}>
                  {RenderLoads()}
                  <XYPlot className='SupportImggraph'
                    width={600}
                    height={100}
                  >
                    <XAxis />
                    <LineSeries
                      data={data}
                      style={{ stroke: 'black', strokeWidth: 0 }}
                    />
                  </XYPlot>
                  <br />
                  <br />
                  <br />
                  {RenderDistributedLoads()}
                  <XYPlot className='SupportImggraph'
                    width={600}
                    height={100}
                  >
                    <XAxis />
                    <LineSeries
                      data={data}
                      style={{ stroke: 'black', strokeWidth: 0 }}
                    />
                  </XYPlot>
                </div>

              )}
            </div>
            <div className='structure-analysis-calculator-calculator-right'>
              <br />
              {selectedOption === 'option1' && (
                <>
                  <LengthOfBeam
                    MetricOrImperial={MetricOrImperial}
                    MetricOrImperialBtn={toggleMetricOrImperial}

                    MetricBeamlengthvalue={metriclength}
                    MetricLengthOfBeamunits={MetricUnits}
                    MetricLengthOfBeamSelectedUnit={metriclengthSelectedUnit}
                    MetrichandleLengthOfBeamChange={((e) => handleMetricInputChange(e.target.value))}
                    MetrichandleLengthOfBeamUnitchange={((e) => handleMetricUnitChange(e.target.value))}
                    MetricBeamLengthoptions={
                      LengthOfBeamunits.map((unit) => (
                        <option key={unit} value={unit}>
                          {unit}
                        </option>
                      ))
                    }


                    ImperialBeamlengthvalue={Imperiallength}
                    ImperialLengthOfBeamunits={ImperialLengthunits}
                    ImperialLengthOfBeamSelectedUnit={ImperiallengthSelectedUnit}
                    ImperialhandleLengthOfBeamChange={((e) => handleImperialInputChange(e.target.value))}
                    ImperialhandleLengthOfBeamUnitchange={((e) => handleImperialUnitChange(e.target.value))}
                    ImperialBeamLengthoptions={
                      ImperialLengthunits.map((unit) => (
                        <option key={unit} value={unit}>
                          {unit}
                        </option>
                      ))
                    }
                    BeamlengthonChange={(e) => setLength(parseFloat(e.target.value))}
                    onChange={handleCrossSectionShapeChange}

                  />
                  <SolutionsOfBeamDeflaction
                    graphText={`${unitText}`}
                    graphText2={`${unitText2}`}

                    Reaction={renderSupportsAndLoadsforReactions()}
                    supports={Supports}
                    DataofShearStressPlot={<Line type="monotone" dataKey={shear} stroke="#8884d8" />}
                    // DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
                    DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
                    DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
                    reactions={(e) => calculateReactions(e.target.value)}
                    DataofReactionPlot={
                      <LineSeries
                        data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
                        style={{ strokeWidth: 0 }}
                      />
                    }
                    NumberlineofReactionplot={
                      <LineSeries
                        data={data}
                        style={{ stroke: 'black', strokeWidth: 0 }}
                      />
                    }
                    LabelSeriesOfReactionForce={
                      <LabelSeries
                        data={[
                          { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
                          { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
                        ]}
                        style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
                      />
                    }

                    CrossSectionShape={CrossSectionShape}

                    togglefunction={isActive3}
                    toggleBreaks={toggleClass3}
                  />
                </>
              )}
              {selectedOption === 'option2' && (
                <>
                  <YoungsModules
                    MetricOrImperial={MetricOrImperial}
                    MetricOrImperialBtn={toggleMetricOrImperial}

                    YoungsModulesValue={E}
                    YoungsModulesSelectedValue={selectedUnit}
                    YoungsModulesValueOnchange={(e) => handleInputChange(e.target.value)}
                    YoungsModulesSelectedValueOnchange={(e) => handleUnitChange(e.target.value)}
                    YoungsModulesOptions={
                      units.map((unit) => (
                        <option key={unit} value={unit}>
                          {unit}
                        </option>
                      ))}


                    ImperialE={ImperialE}
                    ImperialselectedUnit={ImperialselectedUnit}
                    handleImperialEInputChange={(e) => handleImperialEInputChange(e.target.value)}
                    handleImperialEUnitChange={(e) => handleImperialEUnitChange(e.target.value)}
                    ImperialYoungsModulesOptions={
                      Imperialunits.map((unit) => (
                        <option key={unit} value={unit}>
                          {unit}
                        </option>
                      ))}
                  />
                  <SolutionsOfBeamDeflaction
                    graphText={`${unitText}`}
                    graphText2={`${unitText2}`}

                    Reaction={renderSupportsAndLoadsforReactions()}

                    DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
                    DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
                    DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
                    reactions={(e) => calculateReactions(e.target.value)}
                    DataofReactionPlot={
                      <LineSeries
                        data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
                        style={{ strokeWidth: 0 }}
                      />
                    }
                    NumberlineofReactionplot={
                      <LineSeries
                        data={data}
                        style={{ stroke: 'black', strokeWidth: 0 }}
                      />
                    }
                    LabelSeriesOfReactionForce={
                      <LabelSeries
                        data={[
                          { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
                          { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
                        ]}
                        style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
                      />
                    }

                    CrossSectionShape={CrossSectionShape}

                    togglefunction={isActive3}
                    toggleBreaks={toggleClass3}
                  />
                </>
              )}
              {selectedOption === 'option3' && (
                <>
                  <CrossSection
                    MetricOrImperial={MetricOrImperial}
                    MetricOrImperialBtn={toggleMetricOrImperial}
                    // A={A}
                    // AOnchange={(e) => handleAChange(e.target.value)}
                    MetricAUnits={MetricAUnits}
                    metricAInputValue={metricAInputValue}
                    metricASelectedUnit={metricASelectedUnit}
                    handleMetricAInputChange={(e) => handleMetricAInputChange(e.target.value)}
                    handleMetricAUnitChange={(e) => handleMetricAUnitChange(e.target.value)}


                    ImperialAunits={ImperialAunits}
                    ImperialAinputValue={ImperialAinputValue}
                    ImperialAselectedUnit={ImperialAselectedUnit}
                    handleImperialAInputChange={(e) => handleImperialAInputChange(e.target.value)}
                    handleImperialAUnitChange={(e) => handleImperialAUnitChange(e.target.value)}


                    // I={I}
                    // IOnchange={(e) => handleIChange(e.target.value)}


                    MetricIUnits={MetricIUnits}
                    metricIInputValue={metricIInputValue}
                    metricISelectedUnit={metricISelectedUnit}
                    handleMetricIInputChange={(e) => handleMetricIInputChange(e.target.value)}
                    handleMetricIUnitChange={(e) => handleMetricIUnitChange(e.target.value)}


                    ImperialIunits={ImperialIunits}
                    ImperialIinputValue={ImperialIinputValue}
                    ImperialIselectedUnit={ImperialIselectedUnit}
                    handleImperialIInputChange={(e) => handleImperialIInputChange(e.target.value)}
                    handleImperialIUnitChange={(e) => handleImperialIUnitChange(e.target.value)}
                  />
                  <SolutionsOfBeamDeflaction
                    graphText={`${unitText}`}
                    graphText2={`${unitText2}`}

                    Reaction={renderSupportsAndLoadsforReactions()}

                    DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
                    DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
                    DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
                    reactions={(e) => calculateReactions(e.target.value)}
                    DataofReactionPlot={
                      <LineSeries
                        data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
                        style={{ strokeWidth: 0 }}
                      />
                    }
                    NumberlineofReactionplot={
                      <LineSeries
                        data={data}
                        style={{ stroke: 'black', strokeWidth: 0 }}
                      />
                    }
                    LabelSeriesOfReactionForce={
                      <LabelSeries
                        data={[
                          { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
                          { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
                        ]}
                        style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
                      />
                    }

                    CrossSectionShape={CrossSectionShape}

                    togglefunction={isActive3}
                    toggleBreaks={toggleClass3}
                  />
                </>
              )}
              {selectedOption === 'option4' && (
                <>
                  <SupportPage

                    addSupport={<button className='BeamLinkBtntoBeamProperies-Addsupport' onClick={addSupport}>Add Support</button>}
                    Supports={Supports.map((support, index) => (
                      <div key={index} style={{ width: '100%', display: 'flex', justifyContent: 'space-evenly', alignItems: 'center' }}>
                        <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
                          <br />
                          <div className='input-and-select-div' style={{ justifyContent: 'center' }}>
                            <select
                              value={support.type}
                              onChange={(e) => {
                                const newSupports = [...Supports];
                                newSupports[index].type = e.target.value;
                                setSupports(newSupports);
                              }}
                              style={{
                                outline: 'none',
                                border: 'none'
                              }}
                            >
                              <option value="pinned">Pinned</option>
                              <option value="roller">Roller</option>
                              <option value="fixed">Fixed</option>
                            </select>
                          </div>
                        </div>
                        <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
                          <br />
                          <div className='input-and-select-div' style={{ justifyContent: 'center' }}>
                            <input
                              style={{
                                border: 'none',
                                float: 'right',
                                textAlign: 'center',
                                width: '70%',
                              }}
                              type="number"
                              value={support.position}
                              onChange={(e) => {
                                const newSupports = [...Supports];
                                newSupports[index].position = Number(e.target.value);
                                setSupports(newSupports);
                              }}
                              max={Length}
                            />
                          </div>
                        </div>
                        <MdDelete style={{ color: 'white', fontSize: '1.5vw' }} onClick={() => removeSupport(index)}>..</MdDelete>
                      </div>
                    ))}
                  />
                  <SolutionsOfBeamDeflaction
                    graphText={`${unitText}`}
                    graphText2={`${unitText2}`}

                    Reaction={renderSupportsAndLoadsforReactions()}

                    DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
                    DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
                    DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
                    reactions={(e) => calculateReactions(e.target.value)}
                    DataofReactionPlot={
                      <LineSeries
                        data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
                        style={{ strokeWidth: 0 }}
                      />
                    }
                    NumberlineofReactionplot={
                      <LineSeries
                        data={data}
                        style={{ stroke: 'black', strokeWidth: 0 }}
                      />
                    }
                    LabelSeriesOfReactionForce={
                      <LabelSeries
                        data={[
                          { x: Supports[0].position, y: 100, label: `${reactionForce} Kn`, style: { fill: 'red' } },
                          { x: Supports[1].position, y: 100, label: `${momentForce} Kn.m`, style: { fill: 'red' } }
                        ]}
                        style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
                      />
                    }

                    CrossSectionShape={CrossSectionShape}

                    togglefunction={isActive3}
                    toggleBreaks={toggleClass3}
                  />
                </>
              )}
              {selectedOption === 'option5' && (
                <>
                  <LoadPage


                    max={Length}
                    AddDistributedLoad={
                      <button className="BeamLinkBtntoBeamProperies-AddLoad BeamLinkBtntoBeamProperies" onClick={addDistributedLoad}>Add +</button>
                    }
                    DistributedLoadStart={Varyingdistributedload.start}
                    DistributedLoadStartOnchange={(e) => handleLoadChange('start', e.target.value)}
                    DistributedLoadend={Varyingdistributedload.end}
                    DistributedLoadendOnchange={(e) => handleLoadChange('end', e.target.value)}
                    DistributedLoadvalueStart={Varyingdistributedload.startMagnitude}
                    DistributedLoadvalueStartOnchange={(e) => handleLoadChange('startMagnitude', e.target.value)}
                    DistributedLoadvalueEnd={Varyingdistributedload.endMagnitude}
                    DistributedLoadvalueEndOnchange={(e) => handleLoadChange('endMagnitude', e.target.value)}

                    AddPointLoad={
                      <button className='BeamLinkBtntoBeamProperies-AddLoad BeamLinkBtntoBeamProperies' onClick={addPointLoad}>Add +</button>}
                    PointLoad={
                      pointLoads.map((load, index) => (
                        <>
                          <h3 style={{ marginBottom: '10px' }}>Point Load {index + 1}</h3>
                          <div style={{ width: '100%', display: 'flex', justifyContent: 'space-evenly', alignItems: 'center' }}>
                            <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
                              <div className='input-and-select-div'
                                style={{ justifyContent: 'center' }}
                              >
                                <input
                                  style={{
                                    border: 'none',
                                    textAlign: 'center',
                                    width: '100%',
                                    display: 'flex',
                                    justifyContent: 'center',
                                    alignItems: 'center'

                                  }}
                                  type='number'
                                  value={load.position}
                                  onChange={(e) => handlePointLoadChange(index, 'position', e.target.value)}
                                  step='1'
                                  min='0'
                                  max={Length}
                                  placeholder='Position'
                                />

                              </div>
                            </div>
                            <div className='Calculator-Side-A' style={{ width: '25%', justifyContent: 'center' }}>
                              <div className='input-and-select-div'
                                style={{ justifyContent: 'center', width: '100%' }}
                              >
                                <input
                                  style={{
                                    border: 'none',
                                    textAlign: 'center', width: '100%',
                                    display: 'flex',
                                    justifyContent: 'center',
                                    alignItems: 'center'

                                  }}
                                  type='number'
                                  value={load.magnitude}
                                  onChange={(e) => handlePointLoadChange(index, 'magnitude', e.target.value)}
                                  placeholder='Magnitude'
                                />


                              </div>
                            </div>
                            <MdDelete style={{ color: 'white', fontSize: '1.5vw' }} onClick={() => removePointLoad(index)}>..</MdDelete>
                          </div>
                        </>
                      ))}


                    MetricOrImperial={MetricOrImperial}
                    MetricOrImperialBtn={toggleMetricOrImperial}

                    MetricBeamlengthvalue={metriclength}
                    MetricLengthOfBeamunits={MetricUnits}
                    MetricLengthOfBeamSelectedUnit={metriclengthSelectedUnit}
                    MetrichandleLengthOfBeamChange={((e) => handleMetricInputChange(e.target.value))}
                    MetrichandleLengthOfBeamUnitchange={((e) => handleMetricUnitChange(e.target.value))}
                    MetricBeamLengthoptions={
                      LengthOfBeamunits.map((unit) => (
                        <option key={unit} value={unit}>
                          {unit}
                        </option>
                      ))
                    }




                    ImperialBeamlengthvalue={Imperiallength}
                    ImperialLengthOfBeamunits={ImperialLengthunits}
                    ImperialLengthOfBeamSelectedUnit={ImperiallengthSelectedUnit}
                    ImperialhandleLengthOfBeamChange={((e) => handleImperialInputChange(e.target.value))}
                    ImperialhandleLengthOfBeamUnitchange={((e) => handleImperialUnitChange(e.target.value))}
                    ImperialBeamLengthoptions={
                      ImperialLengthunits.map((unit) => (
                        <option key={unit} value={unit}>
                          {unit}
                        </option>
                      ))
                    }

                  // Beamlengthvalue={length}
                  // LengthOfBeamunits={LengthOfBeamunits}
                  // LengthOfBeamSelectedUnit={LengthOfBeamSelectedUnit}
                  // handleLengthOfBeamChange={((e) => handleLengthOfBeamChange(e.target.value))}
                  // handleLengthOfBeamUnitchange={((e) => handleLengthOfBeamUnitchange(e.target.value))}
                  // BeamLengthoptions={
                  // LengthOfBeamunits.map((unit) => (
                  // <option key={unit} value={unit}>
                  // {/* {unit} */}
                  // {/* </option> */}
                  // ))
                  // }
                  />
                  <SolutionsOfBeamDeflaction
                    LoadBendingDeflectionclasses={'LoadBendingDeflectionclasses'}
                    graphText={`${unitText}`}
                    graphText2={`${unitText2}`}

                    Reaction={renderSupportsAndLoadsforReactions()}

                    DataofShearStressPlot={<Line data={dataPoints.map(x => ({ x, y: calculateShearForce(x) }))} />}
                    DataofBendingMomentPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateBendingMoment(x) }))} />}
                    DataofDeflactionPlot={<LineSeries data={dataPoints.map(x => ({ x, y: calculateDeflection(x) }))} />}
                    reactions={(e) => calculateReactions(e.target.value)}
                    DataofReactionPlot={
                      <LineSeries
                        data={[{ x: 0, y: 0 }, { x: Length, y: 0 }]}
                        style={{ strokeWidth: 0 }}
                      />
                    }
                    NumberlineofReactionplot={
                      <LineSeries
                        data={data}
                        style={{ stroke: 'black', strokeWidth: 0 }}
                      />
                    }
                    LabelSeriesOfReactionForce={
                      <LabelSeries
                        data={[
                          { x: Supports[0].position, y: 100, label: `${reactionForce} KN`, style: { fill: 'red' } },
                          { x: Supports[1].position, y: 100, label: `${momentForce} KN.m`, style: { fill: 'red' } }
                        ]}
                        style={{ fontSize: '12px', transform: 'translate(30px, 10px)' }}
                      />
                    }

                    CrossSectionShape={CrossSectionShape}

                    togglefunction={isActive3}
                    toggleBreaks={toggleClass3}
                  />
                </>
              )}
            </div>
          </section>

          <br />
          <br />
          <br />
          <br />
          <div className={isActive3 ? ' Beamdeflaction height110 ' : ' Beamdeflaction height110 height0 '} ></div>
          <hr className='Beam-properties-calculator-hr' />
          <br />
          <br />
          <br />
          <br />
          <BeamDeflection />
          <br />
          <br />
          <br />
          <hr className='Beam-properties-calculator-hr' />
          <br />
          <br />
          <br />
          <br />
          <br />
          <Load />
          <hr className='Beam-properties-calculator-hr' />
          <br />
          <br />
          <br />
          <br />
          <br />
          <br />
        </section>
      </section>
      <footer className='footer'></footer>
    </>
  )
}