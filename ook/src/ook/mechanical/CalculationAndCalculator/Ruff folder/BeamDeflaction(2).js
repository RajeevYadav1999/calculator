...
export default function BeamDeflectionCalculator() {
  ...
  const [pointLoads, setPointLoads] = useState([{ position: 1000, value: 800 }]);

  const [load, setLoad] = useState({ start: 0, end: length, valueStart: 100, valueEnd: 1000 });
  ...
  const addDistributedLoad = () => {
    setDistributedLoads([...distributedLoads, { start: 0, end: length, startMagnitude: 0, endMagnitude: 0 }]);
  };
  ...
  const calculateShearForce = (x) => {
    ...
    distributedLoads.forEach(load => {
      if (x >= load.start) shear -= load.startMagnitude * (Math.min(x, load.end) - load.start);
    });
    return shear;
  };
  ...
  const calculateBendingMoment = (x) => {
    ...
    distributedLoads.forEach(load => {
      if (x >= load.start) {
        const a = Math.min(x, load.end) - load.start;
  ...
  const calculateDeflection = (x) => {
    ...
    for (let i = 0; i < x; i += 0.1) {
      deflection += (1 / EI) * calculateBendingMoment(i) * 0.1;
    }
  ...
  const reactionForce = pointLoads[0].value;
  const momentForce = -pointLoads[0].value * (pointLoads[0].position - (Supports[1].position));
              ...
              {selectedOption === 'option5' && (
                <>
                  <LoadPage
                    ...
                    DistributedLoadvalueStart={load.valueStart}
                    DistributedLoadvalueStartOnchange={(e) => handleLoadChange('valueStart', e.target.value)}
                    DistributedLoadvalueEnd={load.valueEnd}
                    DistributedLoadvalueEndOnchange={(e) => handleLoadChange('valueEnd', e.target.value)}
