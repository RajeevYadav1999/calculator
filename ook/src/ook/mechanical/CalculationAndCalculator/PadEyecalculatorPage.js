...
export default function PadEyecalculatorPage() {
    ...
    const ShackleInsideLengthvalue = parseFloat(internalShackleInsideLength);
    const LengthClearanceofShacklePlusvalue = ShackleInsideLengthvalue + (internalShacklePinDiameter / 2)
    const LengthClearanceofShackleminusValue = internalRadiusofMainPlateValue - (- internalSlingDiameter);
    // const LengthClearanceofShackle = Plusvalue - minusValue;
    const LengthClearanceofShackleValue = (LengthClearanceofShacklePlusvalue - LengthClearanceofShackleminusValue).toFixed(3);
    ...
    const BearingStressActualLoadonPadeye = parseFloat(LoadonPadeyeValueInMT);
    const BearingStressActualDLF = parseFloat(DLFValue)
    const BearingStressActualPadEyeDesignLoad = BearingStressActualLoadonPadeye * BearingStressActualDLF;
    const BearingStressActualDp = parseFloat(internalShacklePinDiameter);
    const BearingStressActualt = parseFloat(internalThicknessofMainPlateValue);
    const BearingStressActualtc = parseFloat(internalThicknessofCheekPlateInputValue);
    const BearingStressActualBearingArea = piby4 * BearingStressActualDp * (BearingStressActualt + 2 * BearingStressActualtc)
    const BearingStressActualValue = (BearingStressActualPadEyeDesignLoad / BearingStressActualBearingArea * 9810);
    // setBearingStressActual(BearingStressActual.toFixed(2));
    // Bearing Stress Check at Pin Hole
    // BearingStressCheckatPinHole
    if (BearingStressActualValue < MaterialYieldStressInMPa * 0.9) {
    ...
    const tc = parseFloat(internalThicknessofCheekPlateInputValue);
    const DLF = parseFloat(DLFValue)
    const ShearAreaofpinholeValue = 2 * ((R - De / 2) * t + 2 * (Rc - De / 2) * tc);
    const pd = LoadonPadeye * DLF;
    const ShearStressatpinholeValue = (pd * 9810 / ShearAreaofpinholeValue).toFixed(2)
    // ShearStressCheckatPinHole
    if (ShearStressatpinholeValue < MaterialYieldStressInMPa * 0.4) {
    ...
    const tValue = parseFloat(internalThicknessofMainPlateValue);
    const RValue = parseFloat(internalRadiusofMainPlateValue);
    const DiameterofCheekPlate = parseFloat(internalDiameterofCheekPlateInputValue);
    const RadiusofCheekPlateValue = DiameterofCheekPlate / 2;
    const RcValue = parseFloat(RadiusofCheekPlateValue);
    const DeValue = parseFloat(internalDiameterofeyepinholeValue);
    const tcValue = parseFloat(internalThicknessofCheekPlateInputValue);
    const TensileAreaforsectionAAValue = 2 * ((RValue - DeValue / 2) * tValue + 2 * (RcValue - DeValue / 2) * tcValue);
    const TensileStressatpinholeActualatsectionAAValue = pd * 9810 / TensileAreaforsectionAAValue;


    const PadEyeDesignLoadValue = LoadonPadeye * DLF;
    const TensileAreaforsectionBBValue = ((2 * R) + (pi * Rc / 2)) * t
    const TensileStressatpinholeActualatsectionBBValue = PadEyeDesignLoadValue * 9810 / TensileAreaforsectionBBValue
    // TensileStressCheckatPinHole
    if (Math.max(TensileStressatpinholeActualatsectionAAValue, TensileStressatpinholeActualatsectionBBValue) < MaterialYieldStressInMPa * 0.45) {
    ...
    const D41 = parseFloat(internalDiameterofeyepinholeValue)
    const D42 = parseFloat(internalShacklePinDiameter)
    const HertzStressatPinHoleValue = (Math.sqrt(D38 * 9810 * D39 * (D41 - D42) / (PI * (1 - Math.pow(D40, 2)) * D41 * D42))).toFixed(3);

    if (HertzStressatPinHoleValue < MaterialYieldStressInMPa * 2.5) {
    ...
    // Stress Checks at Base Plate
    const multiplyValue = Math.cos(AngleofLoadwithVerticalvalue * Math.PI / 180);
    const LoadinVerticalDirectionZdirection = LoadonPadeye * multiplyValue;
    const ValueofDesignLoadinVerticalDirectionZdirection = LoadinVerticalDirectionZdirection * DLF;
    const pt = parseFloat(ValueofDesignLoadinVerticalDirectionZdirection)
    const L = parseFloat(internalLengthofBasePlateInputValue)
    const T = parseFloat(internalThicknessofMainPlateValue)
    const TensileAreavalue = L * T;
    const atb = parseFloat(TensileAreavalue)
    const ValueofTensileStressatBaseActual = pt * 9810 / atb;
    // Tensile Stress Check at Base
    if (ValueofTensileStressatBaseActual < MaterialYieldStressInMPa * 0.6) {
    ...
    const H = parseFloat(internalinputTotalHeightofPadeyeValue)
    const LoadinHorizontalDirectionYdirectionMultipleValue = Math.sin(AngleofLoadwithVerticalvalue * pi / 180) * Math.cos(OutofPlaneAngleValue * pi / 180);
    const LoadinHorizontalDirectionYdirectionValue = (LoadonPadeyeValueInMT * LoadinHorizontalDirectionYdirectionMultipleValue).toFixed(1);
    const pyd = (LoadinHorizontalDirectionYdirectionValue * DLFValue).toFixed(2)
    const LoadinVerticalDirectionZdirectionValuemultiplyValue = Math.cos(AngleofLoadwithVerticalvalue * Math.PI / 180);
    const pzdLoadinVerticalDirectionZdirectionValue = LoadonPadeye * LoadinVerticalDirectionZdirectionValuemultiplyValue;
    const h = H - R
    const pzd = pzdLoadinVerticalDirectionZdirectionValue * DLF;
    const MomentleverforVerticalForceValue = R - (L / 2);
    const e = parseFloat(MomentleverforVerticalForceValue)
    const Mdxx = pyd * h + pzd * e;
    // const L = parseFloat(internalLengthofBasePlateInputValue)
    const Zbxx = T * Math.pow(L, 2) / 6;
    const ValueofBendingStressActualaboutXaxisInplane = Mdxx * 9810 / Zbxx;

    if (ValueofBendingStressActualaboutXaxisInplane < MaterialYieldStressInMPa * 0.6) {
    ...
    if (ValueofBendingStressActualaboutYaxisOutofPlane < MaterialYieldStressInMPa * 0.75) {
    ...
    const LoadinHorizontalDirectionYdirectionLoadinHorizontalDirectionYdirectionMultipleValue = Math.sin(AngleofLoadwithVerticalvalue * pi / 180) * Math.cos(OutofPlaneAngleValue * pi / 180);
    const pydLoadinHorizontalDirectionYdirection = (LoadonPadeyeValueInMT * LoadinHorizontalDirectionYdirectionLoadinHorizontalDirectionYdirectionMultipleValue).toFixed(1);
    const ValueofShearStressatBaseActualHorizontalYdirectionpyd = (pydLoadinHorizontalDirectionYdirection * DLFValue).toFixed(2)
    const ValueofShearStressatBaseActualHorizontalYdirectionL = parseFloat(internalLengthofBasePlateInputValue)
    const ValueofShearStressatBaseActualHorizontalYdirectionT = parseFloat(internalThicknessofMainPlateValue)
    const asb = ValueofShearStressatBaseActualHorizontalYdirectionL * ValueofShearStressatBaseActualHorizontalYdirectionT
    const ValueofShearStressatBaseActualHorizontalYdirection = (ValueofShearStressatBaseActualHorizontalYdirectionpyd * 9810 / asb).toFixed(3);
    if (ValueofShearStressatBaseActualHorizontalYdirection < MaterialYieldStressInMPa * 0.4) {
    ...
    const pxdPadEyeDesignLoadvalue = LoadonPadeyeValueInMT * DLF;
    const pxdOutofplaneLoadLateralLoadXdirectionMultiplyValue = Math.sin(AngleofLoadwithVerticalvalue * pi / 180) * Math.sin(OutofPlaneAngleValue * pi / 180)
    const pxdOutofplaneLoadLateralLoadXdirection = LoadonPadeye * pxdOutofplaneLoadLateralLoadXdirectionMultiplyValue;
    const ValueShearStressatBaseActualLateralXdirectionpxd = (pxdOutofplaneLoadLateralLoadXdirection * DLF) + (0.05 * pxdPadEyeDesignLoadvalue);
    const ValueShearStressatBaseActualLateralXdirectionL = parseFloat(internalLengthofBasePlateInputValue)
    const ValueShearStressatBaseActualLateralXdirectionT = parseFloat(internalThicknessofMainPlateValue)
    const ValueShearStressatBaseActualLateralXdirectionasb = ValueShearStressatBaseActualLateralXdirectionL * ValueShearStressatBaseActualLateralXdirectionT
    const ValueShearStressatBaseActualLateralXdirection = (ValueShearStressatBaseActualLateralXdirectionpxd * 9810 / ValueShearStressatBaseActualLateralXdirectionasb).toFixed(2);

    if (ValueShearStressatBaseActualLateralXdirection < 35) {
    ...
    const ValueofLoadinVerticalDirectionZdirectionValuemultiplyValue = Math.cos(AngleofLoadwithVerticalvalue * Math.PI / 180);
    const ValueofDesignLoadinVerticalDirectionZdirectionLoadinVerticalDirectionZdirectionValue = LoadonPadeye * ValueofLoadinVerticalDirectionZdirectionValuemultiplyValue;
    const DesignLoadinVerticalDirectionZdirectionValue = ValueofDesignLoadinVerticalDirectionZdirectionLoadinVerticalDirectionZdirectionValue * DLF;
    const ValueofTensileStressatBaseActualpt = parseFloat(DesignLoadinVerticalDirectionZdirectionValue)
    const ValueofTensileStressatBaseActualL = parseFloat(internalLengthofBasePlateInputValue)
    const ValueofTensileStressatBaseActualT = parseFloat(internalThicknessofMainPlateValue)
    const ValueofTensileAreavalue = ValueofTensileStressatBaseActualL * ValueofTensileStressatBaseActualT;
    const Valueofatb = parseFloat(ValueofTensileAreavalue)
    const ValueTensileStressatBaseActual = ValueofTensileStressatBaseActualpt * 9810 / Valueofatb;
    const fa = ValueTensileStressatBaseActual

    const ft = MaterialYieldStressInMPa * 0.6


    const ValueBendingStressActualaboutXaxisInplane = Mdxx * 9810 / Zbxx;
    const fbx = ValueBendingStressActualaboutXaxisInplane
    const Fbx = MaterialYieldStressInMPa * 0.6

    const b = parseFloat(internalThicknessofMainPlateValue)
    const d = parseFloat(internalLengthofBasePlateInputValue)
    const tw = parseFloat(internalBaseWeldLegSize)
    const ValueofZwbyy = (b * d + Math.pow(b, 2) / 3) * tw;
    const ValueZwbyy = parseFloat(ValueofZwbyy)
    const ValueBendingStressaboutYaxisOutofPlane = (Mdyy * 9810 / Zbyy)
    const fby = ValueBendingStressaboutYaxisOutofPlane
    const Fby = MaterialYieldStressInMPa * 0.75
    ...
    const Asbw = twt * Lw
    const Pyd = LoadinVerticalDirectionZdirectionValue * DLF;
    const ValueofTensileStressatBaseWeldActual = ((Pyd * 9810) / Asbw).toFixed(3)
    const σtbwAct = parseFloat(ValueofTensileStressatBaseWeldActual)
    ...
    const ValueofBendingStressatBaseWeldActualaboutXAxisInPlane = (ValueofMdxx * 9810 / Zwbxx).toFixed(3)
    const σbbwxAct = parseFloat(ValueofBendingStressatBaseWeldActualaboutXAxisInPlane)


    const ZwbyyValue = (b * d + Math.pow(b, 2) / 3) * tw;
    const MdxxValue = pxd * h;
    const Zwbyy = parseFloat(ZwbyyValue)
    const ValueofBendingStressaboutYaxisOutofPlane = (MdxxValue * 9810 / Zwbyy).toFixed(3)
    const σbbyAct = parseFloat(ValueofBendingStressaboutYaxisOutofPlane)
    ...
    const ShearStressatBaseWeldActualHorizontalYdirectionAsbw = ShearStressatBaseWeldActualHorizontalYdirectiontwt * ShearStressatBaseWeldActualHorizontalYdirectionLw
    const ShearStressatBaseWeldActualHorizontalYdirectionPyd = ShearStressatBaseWeldActualHorizontalYdirectionLoadinHorizontalDirectionYdirection * DLFValue
    const ValueofShearStressatBaseWeldActualHorizontalYdirection = ((ShearStressatBaseWeldActualHorizontalYdirectionPyd * 9810) / ShearStressatBaseWeldActualHorizontalYdirectionAsbw).toFixed(2)
    const τsbwyAct = parseFloat(ValueofShearStressatBaseWeldActualHorizontalYdirection)



    const LoadinVerticalDirectionZdirectionmultiplyValue = Math.cos(AngleofLoadwithVerticalvalue * Math.PI / 180);
    const LoadinVerticalDirectionZdirectioninput = (LoadonPadeyeValueInMT * LoadinVerticalDirectionZdirectionmultiplyValue).toFixed(2);
    const Pxd = (OutofplaneLoadLateralLoadXdirection * DLF) + (0.05 * PadEyeDesignLoadvalue)
    const ValueofShearStressatBaseWeldActualLateralXdirection = ((Pxd * 9810) / Asbw).toFixed(3)
    const τsbwxAct = parseFloat(ValueofShearStressatBaseWeldActualLateralXdirection)
    const ValueofTotalStressatBaseWeldActual = (Math.sqrt(Math.pow(σtbwAct + σbbwxAct + σbbyAct, 2) + Math.pow(τsbwyAct, 2) + Math.pow(τsbwxAct, 2))).toFixed(3);
    if (ValueofTotalStressatBaseWeldActual < ElectrodeTensileStrengthInMpa * 0.3) {
    ...
    if (ValueofWeldStressatCheekWeldActual < ElectrodeTensileStrengthInMpa * 0.3) {
